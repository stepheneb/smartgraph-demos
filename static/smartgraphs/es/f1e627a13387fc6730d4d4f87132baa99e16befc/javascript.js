...
}});Smartgraphs.GraphController=SC.ObjectController.extend(SC.Responder,{seriesList:null,selectedSeries:null,annotationList:null,_routeEvents:NO,eventQueue:[],freehandIsOn:NO,openGraph:function(a){var e=Smartgraphs.store.find(Smartgraphs.Graph,a);
if(!e){e=Smartgraphs.store.createRecord(Smartgraphs.Graph,{id:a});Smartgraphs.store.commitRecords()
}this.set("content",e);this.set("seriesList",[]);this.set("annotationList",[]);var b=this.get("initialSeries");
for(var c=0,d=b.get("length");c<d;c++){this.addSeriesByName(b.objectAt(c))}},setAxes:function(a){var b=Smartgraphs.store.find(Smartgraphs.Axes,a);
if(!b){b=Smartgraphs.store.createRecord(Smartgraphs.Axes,{guid:a})}this.set("axes",b);
Smartgraphs.store.commitRecords()},addSeries:function(a){if(this.findSeries(a.get("name"))){return NO
}this.get("seriesList").pushObject(a);Smartgraphs.store.commitRecords();return YES
},addSeriesByName:function(a){var c=SC.Query.local(Smartgraphs.DataSeries,"name={name} AND session={session}",{name:a,session:Smartgraphs.sessionController.getPath("content")});
var b=Smartgraphs.store.find(c);if(b.get("length")<1){c=SC.Query.local(Smartgraphs.DataSeries,"name={name} AND isExample=YES",{name:a});
b=Smartgraphs.store.find(c);if(b.get("length")<1){return NO}}this.addSeries(b.objectAt(0))
},removeSeries:function(a){var c=this.get("seriesList");var b=this.findSeries(a);
if(b){c.removeObject(b)}},findSeries:function(a){var d=this.get("seriesList");var c;
for(var b=0,e=d.get("length");b<e;b++){c=d.objectAt(b);if(c.get("name")===a){return c
}}},findAnnotationByName:function(d){var e=this.get("annotationList");var a;for(var b=0,c=e.get("length");
b<c;b++){a=e.objectAt(b);if(a.get("name")===d){return a}}return null},selectSeries:function(a){var b=this.findSeries(a);
...
}var c=b?b.findAnnotationByName(a):NO;if(c&&SC.kindOf(c,Smartgraphs.TraceAnnotation)){this._graphController=b;
this._trace=c;return YES}return NO},startInput:function(){if(!this._trace){return NO
...
this._graphController.endFreehandInput();this._graphController=null;this._trace=null;
this._inputStarted=NO},graphObserver:function(){var b,a=this._graphController.get("eventQueue");
while((b=a.shiftObject())){switch(b.type){case this.CONTINUE:break;case this.START:break;
case this.END:break}}},nBins:50,xMin:null,xMax:null,_points:[],binWidth:function(){return((this.get("xMax")-this.get("xMin"))/this.get("nBins"))
...
var m=[];for(d=0,n=this.get("length");d<n;d++){var l=this._series.get("points").objectAt(d);
m.push(l.get("id"));k=l.get("x");h=l.get("y");var b=Math.floor((k-c)/a);if(0<=b&&b<j){this._bins[b].push(h)
...
}}})},addPredictionPoint:function(b,d,c){var a=Smartgraphs.store.createRecord(Smartgraphs.DataPoint,{x:b,y:d,guid:Smartgraphs.getNextGuid()});
this._series.get("points").pushObject(a);Smartgraphs.store.commitRecords();this._points[c]=a
},_strokeStarted:NO,_lastX:null,_lastY:null,addOrUpdatePredictionPoint:function(b,f){var d=this.get("xMin");
...
}b=(c+0.5)*e;this.invokeLast(function(){this.addPredictionPoint(b,f,c)})},startAt:function(a,d){var b=this.get("xMin"),c=this.get("xMax");
if(a<b||a>c){return YES}this._lastX=a;this._lastY=d;this._strokeStarted=YES;this.addOrUpdatePredictionPoint(a,d);
return YES},continueAt:function(n,k){if(!this._strokeStarted){this.startAt(n,k);return YES
}var c=this.get("xMin"),h=this.get("xMax"),a=this.get("binWidth"),m=this.get("nBins"),l=Math.floor((n-c)/a),g=Math.floor((this._lastX-c)/a);
...
}});this._lastX=n;this._lastY=k},endAt:function(a,d){this._strokeStarted=NO;this._lastX=null;
this._lastY=null;var b=this.get("xMin"),c=this.get("xMax");if(a<b||a>c){return YES
}this.addOrUpdatePredictionPoint(a,d);return YES}});Smartgraphs.freehandInputController.START=1;
...
Smartgraphs.RestDataSource=SC.DataSource.extend({latency:500,fetch:function(a,f){var g,b,e,c;
...
sc_require("models/data_point");Smartgraphs.DataPoint.FIXTURES=[];Smartgraphs.DataSeries=SC.Record.extend({url:SC.Record.attr(String),primaryKey:"url",name:SC.Record.attr(String),session:SC.Record.toOne("Smartgraphs.Session"),isExample:SC.Record.attr(Boolean),points:SC.Record.toMany("Smartgraphs.DataPoint",{inverse:"series"}),defaultDisplayType:SC.Record.attr(Number)});
Smartgraphs.DataSeries.LINE_GRAPH=1;Smartgraphs.DataSeries.SCATTER_PLOT=2;sc_require("models/data_series");
Smartgraphs.DataSeries.FIXTURES=[{url:"/backend/activity/1/series/example-1",name:"example-1",isExample:YES,points:[],session:null}];
Smartgraphs.Graph=SC.Record.extend({url:SC.Record.attr(String),primaryKey:"url",name:SC.Record.attr(String),description:SC.Record.attr(String),axes:SC.Record.toOne("Smartgraphs.Axes"),initialSeries:SC.Record.attr(Array)});
sc_require("models/graph");Smartgraphs.Graph.FIXTURES=[{url:"/backend/activity/1/graph/1/prediction-away",name:"Prediction-Away",description:"Prediction graph of movement away",axes:"/backend/axes/1/5m-15s",initialSeries:[]},{url:"/backend/activity/1/graph/2/prediction-toward",name:"Prediction-Toward",description:"Prediction graph of movement towards",axes:"/backend/axes/1/5m-15s",initialSeries:[]},{url:"/backend/activity/1/graph/3/sensor-playing",name:"Sensor-Playing",description:"Playing around with the sensor in page 3",axes:"/backend/axes/1/5m-15s",initialSeries:[]},{url:"/backend/activity/1/graph/4/combined",name:"Combined",description:"Combines the prediction graphs and the sensor graph",axes:"/backend/axes/1/5m-15s",initialSeries:["prediction-away","sensor"]}];
...
sc_require("models/session_step");Smartgraphs.SessionStep.FIXTURES=[];Smartgraphs.TraceAnnotationView=RaphaelViews.RaphaelView.extend({stroke:"#000000",strokeWidth:2,renderCallback:function(d,a,b,c){return d.path(a).attr({stroke:b,"stroke-width":c})
},render:function(b,a){var c=this.getPath("parentView.parentView");var e=this.get("content");
var h=e?e.get("points"):[{x:0,y:0}];var f=[];var g;for(var d=0,j=h.get("length");
d<j;d++){g=c.coordinatesForPoint(h.objectAt(d));f.push(d===0?"M":"L");f.push(g.x);
f.push(" ");f.push(g.y)}var l=f.join("");if(a){b.callback(l,this.get("stroke"),this.get("strokeWidth"))
}else{var k=b.raphael();k.attr({path:l})}}});sc_require("views/trace_annotation");
Smartgraphs.TraceAnnotation=SC.Record.extend({url:SC.Record.attr(String),primaryKey:"url",isAnnotation:YES,name:SC.Record.attr(String),session:SC.Record.toOne("Smartgraphs.Session"),isExample:SC.Record.attr(Boolean),points:SC.Record.attr(Array),isDirectional:SC.Record.attr(Boolean)});
Smartgraphs.TraceAnnotation.viewClass=Smartgraphs.TraceAnnotationView;sc_require("models/trace_annotation");
Smartgraphs.TraceAnnotation.FIXTURES=[{url:"/backend/activity/1/annotations/trace-1",name:"trace-1",isExample:YES,points:[],session:null}];
...
a.addSeriesByName(b.seriesName);return YES},copyExampleSeriesToGraph:function(d,b){var a=this._graphControllerFor(b.pane);
...
b.addAnnotation(a);return YES},startFreehandInput:function(c,b){Smartgraphs.sendAction("createAnnotation",this,{pane:b.pane,annotationName:b.annotationName,annotationType:Smartgraphs.TraceAnnotation});
...
var a=this._graphControllerFor(b.pane);var c=a.findSeries(b.seriesName);if(c.get("isExample")===NO){Smartgraphs.selectedSeriesController.set("content",c)
...
Smartgraphs.triggers.stepFinished=Smartgraphs.TriggerObserver.create({});Smartgraphs.AxisView=RaphaelViews.RaphaelView.extend({init:function(){if(this.get("type")==="x"){this.set("displayProperties","axes.xMin axes.xMax axes.xSteps".w())
}else{this.set("displayProperties","axes.yMin axes.yMax axes.ySteps".w())}arguments.callee.base.apply(this,arguments)
},render:function(a,b){if(!b){this.drawAxis()}},didCreateLayer:function(){this.invokeLater(this.drawAxis)
},drawAxis:function(){if(this._axis){this._axis.remove()}var h=this.get("axes");if(h){var l=this.getPath("parentView.parentView.parentView.padding");
...
}}});Smartgraphs.GraphView=SC.View.extend({axesBinding:"*graphController.axes",seriesListBinding:"*graphController.seriesList",annotationListBinding:"*graphController.annotationList",padding:{top:20,right:20,bottom:45,left:55},childViews:"graphCanvasView".w(),init:function(){this._seriesViewsById={};
...
},pointForCoordinates:function(l,j){var g=this.get("axes");var b=g.get("xMin"),f=g.get("xMax"),n=g.get("yMin"),e=g.get("yMax");
var d=this.get("frame");var o=d.height,a=d.width;var k=this.get("padding");var i=a-k.left-k.right;
var h=o-k.top-k.bottom;var m=i/(f-b);var c=h/(e-n);return{x:b+(l-k.left)/m,y:n+(k.top+h-j)/c}
},_seriesListDidChange:function(){var d=this.get("seriesList");var e,a;var b={};var h=[],c=[];
for(var f=0,j=d.get("length");f<j;f++){e=d.objectAt(f);a=e.get("id");b[a]=e;if(!this._seriesViewsById.hasOwnProperty(a)){this._addViewForSeries(e)
}}var g;for(a in this._seriesViewsById){if(this._seriesViewsById.hasOwnProperty(a)){g=this._seriesViewsById[a];
if(!b[a]){this._removeSeriesView(g)}}}}.observes("*seriesList.[]"),_addViewForSeries:function(b){var c=SC.Query.local(Smartgraphs.DataPoint,{conditions:"series = {series}",series:b,orderBy:"id"});
...
delete this._seriesViewsById[b];this.get("graphCanvasView").removeChild(a)},_annotationListDidChange:function(){var d=this.get("annotationList");
var a;for(var b=0,c=d.get("length");b<c;b++){a=d.objectAt(b);console.log("graphView adding annotation: ",a.get("name"))
}}.observes("*annotationList.[]"),graphCanvasView:RaphaelViews.RaphaelCanvasView.design({axesBinding:".parentView.axes",displayProperties:"axes.xMin axes.xMax axes.yMin axes.yMax".w(),childViews:"axesView".w(),axesView:RaphaelViews.RaphaelView.design({axesBinding:".parentView.parentView.axes",paddingBinding:".parentView.parentView.padding",childViews:"xLabelView yLabelView xAxisView yAxisView inputArea".w(),inputArea:RaphaelViews.RaphaelView.design({axesBinding:".parentView.parentView.parentView*axes",didCreateLayer:function(){this._graphView=this.getPath("parentView.parentView.parentView");
this._$graphView=this._graphView.$();this._trace=this.get("raphaelCanvas").path("M0 0").attr({stroke:"#000000","stroke-width":2})
},renderCallback:function(e,d,b,c,a){return e.rect(d,b,c,a).attr({fill:"#ffffff",stroke:"#ffffff",opacity:0.7})
...
var c=this.coordsForEvent(b);if(this._graphController.get("freehandIsOn")){this._traceStr="M"+c.x+" "+c.y
}var a=this._graphView.pointForCoordinates(c.x,c.y);return this._graphController.inputAreaMouseDown(a.x,a.y)
},mouseDragged:function(b){var c=this.coordsForEvent(b);if(this._graphController.get("freehandIsOn")){this._traceStr=this._traceStr+" L"+c.x+" "+c.y;
this._trace.attr({path:this._traceStr})}var a=this._graphView.pointForCoordinates(c.x,c.y);
return this._graphController.inputAreaMouseDragged(a.x,a.y)},mouseUp:function(b){var c=this.coordsForEvent(b);
if(this._graphController.get("freehandIsOn")){this._traceStr=this._traceStr+" L"+c.x+" "+c.y;
this._trace.attr({path:this._traceStr})}var a=this._graphView.pointForCoordinates(c.x,c.y);
return this._graphController.inputAreaMouseUp(a.x,a.y)}}),xLabelView:RaphaelViews.RaphaelView.design({axesBinding:".parentView.parentView.parentView.axes",displayProperties:"axes.xLabel".w(),render:function(a,b){if(!b){this.drawLabel()
}},didCreateLayer:function(){this.invokeLater(this.drawLabel)},drawLabel:function(){if(this._label){this._label.remove()
}var a=this.getPath("axes.xLabel");var b=this.getPath("parentView.parentView.parentView.padding");
var c=this.getPath("parentView.parentView.parentView.frame");this._label=this.get("raphaelCanvas").text((b.left+c.width-b.right)/2,c.height-15,a).attr({font:"14px Arial, sans-serif"})
}}),yLabelView:RaphaelViews.RaphaelView.design({axesBinding:".parentView.parentView.parentView.axes",displayProperties:"axes.yLabel".w(),render:function(a,b){if(!b){this.drawLabel()
}},didCreateLayer:function(){this.invokeLater(this.drawLabel)},drawLabel:function(){if(this._label){this._label.remove()
}var a=this.getPath("axes.yLabel");var b=this.getPath("parentView.parentView.parentView.padding");
var c=this.getPath("parentView.parentView.parentView.frame");this._label=this.get("raphaelCanvas").text(15,(b.top+c.height-b.bottom)/2,a).attr({font:"14px Arial, sans-serif"}).rotate(270)

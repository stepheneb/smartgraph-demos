...
})();Smartgraphs=SC.Application.create({NAMESPACE:"Smartgraphs",VERSION:"0.1.0",DATA_FORMAT_VERSION:3,rootStore:function(){var a=Smartgraphs.store;
...
}.property("status")});Smartgraphs.activityObjectsController=SC.ObjectController.create({_annotations:{},_datasets:{},loadPredefinedObjects:function(){this._datasets={};
...
return a},datasetNames:function(){var b=[];for(var a in this._datasets){if(this._datasets.hasOwnProperty(a)){b.push(a)
...
this.clearAnnotations();this.set("content",null);this.set("dataset",null);this.set("datasetName",null)
},openDataset:function(c){var a=this.get("datasetName");if(a===c){return YES}this.clear();
...
}SC.RunLoop.begin().end();var b=Smartgraphs.store.get("changelog")||[];b.forEach(function(c){console.log("    about to permanently turn off observing for about-to-be-destroyed record %d",c);
var d=Smartgraphs.store.find(Smartgraphs.store.recordTypeFor(c),Smartgraphs.store.idFor(c));
if(d){d._notifyPropertyObservers=function(){}}});Smartgraphs.store.discardChanges().destroy();
Smartgraphs.set("store",a);this.set("content",null)}});Smartgraphs.toolbarController=SC.ObjectController.create({shouldShowEditButton:NO,shouldShowRunButton:NO,shouldShowSaveButton:NO,showRunButton:function(){this.set("shouldShowEditButton",NO);
...
});f.activity.datasets=f.datasets.map(function(g){return g.url});f.activity.axes=f.axes.map(function(g){return g.url
});f.activity.graphs=f.graphs.map(function(g){return g.url});f.activity.responseTemplates=f.responseTemplates.map(function(g){return g.url
...
})})}});return e}});Smartgraphs.ActivityPage=SC.Record.extend({url:SC.Record.attr(String),primaryKey:"url",activity:SC.Record.toOne("Smartgraphs.Activity",{inverse:"pages",isMaster:YES,aggregate:YES}),name:SC.Record.attr(String),index:SC.Record.attr(Number),introText:SC.Record.attr(String),steps:SC.Record.toMany("Smartgraphs.ActivityStep",{inverse:"activityPage"}),firstStep:SC.Record.toOne("Smartgraphs.ActivityStep"),isSelectable:NO,pageNumber:null,pageNumberAsString:function(){return(this.get("pageNumber")+1)+""
...
Smartgraphs.Annotation=SC.Record.extend({url:SC.Record.attr(String),primaryKey:"url",isAnnotation:YES,name:SC.Record.attr(String),activity:SC.Record.toOne("Smartgraphs.Activity",{aggregate:YES}),session:SC.Record.toOne("Smartgraphs.Session"),color:SC.Record.attr(String,{defaultValue:"#cc0000"}),isHighlighted:SC.Record.attr(Boolean,{defaultValue:false})});
Smartgraphs.Annotation.types=[];Smartgraphs.Annotation.typeNames=[];Smartgraphs.DataPoint=SC.Record.extend({x:SC.Record.attr(Number),y:SC.Record.attr(Number),dataset:SC.Record.toOne("Smartgraphs.Dataset",{inverse:"points",isMaster:YES,aggregate:YES}),xRounded:function(){return Math.round(this.get("x")*100)/100
...
var b=d.color?d.color:"#000000";var g=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.LineThroughPoints,d.lineName,{point1:f.get("point").get("id"),point2:c.get("point").get("id"),color:b});
...
}else{g=[h.get("id"),e.get("id")]}var a=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.Arrow,d.arrowName,{point1:g[0],point2:g[1],color:c,isVertical:YES,isClockwise:YES});
...
}else{f=[h.get("id"),d.get("id")]}var g=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.Arrow,c.arrowName,{point1:f[0],point2:f[1],color:b,isHorizontal:YES,isClockwise:YES});
return YES},createIndicatingArrowFromDataPoint:function(c,b){var a=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.IndicatingArrow,b.arrowName,{dataPoint:b.point.get("id"),pointAngle:b.angle,color:b.color});
...
}}if(!b){return YES}var e=b.findAnnotationByName(c.point);var a=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.IndicatingArrow,c.arrowName,{annotation:e.get("id"),pointAngle:c.angle?c.angle:335,color:c.color?c.color:"#cc0000",length:c.length?c.length:40});
return YES},createIndicatingArrowFromCoordinates:function(c,b){var a=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.IndicatingArrow,b.arrowName,{specificX:b.x,specificY:b.y,pointAngle:b.angle?b.angle:335,color:b.color?b.color:"#cc0000",length:b.length?b.length:40});
return YES},createBracketArcFromCoordinates:function(c,a){var b=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.BracketArc,a.bracketName,{startX:a.startX,startY:a.startY,endX:a.endX,endY:a.endY,color:a.color?a.color:"#cc0000",isClockwise:false});
...
var e=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.BracketArc,d.bracketName,{startX:310,startY:(b*20)+10,endX:310,endY:(f*20)+10,color:d.color?d.color:"#cc0000",isClockwise:(b<f)});
return YES},createRunBracket:function(g,d){var c=Smartgraphs.TableController.controllerForDataset[d.tableName];
if(!c){return YES}var a=c.findAnnotationByName(d.point1);var h=c.findAnnotationByName(d.point2);
var b=c.get("dataset").get("points").indexOf(a.get("point"));var f=c.get("dataset").get("points").indexOf(h.get("point"));
var e=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.BracketArc,d.bracketName,{startX:40,startY:(b*20)+10,endX:40,endY:(f*20)+10,color:d.color?d.color:"#cc0000",isClockwise:(b>f)});
...
var a=Smartgraphs.activityObjectsController.createAnnotation(c.type,c.name);b.addAnnotation(a);
...
var a=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.HighlightedPoint,b.annotationName,{color:b.color});
...
a.dataset.get("selection").forEach(function(e){var d=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.HighlightedPoint,a.highlightedPointName,{point:e})
...
if(d.get("length")!==1){return YES}var e=d.firstObject();var g=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.HighlightedPoint,b.highlightedPointName,{point:e.get("id")});
...
var c=g.color?g.color:"#000000";var h=Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.LineThroughPoints,g.lineName,{point1:b.get("point").get("id"),point2:f.get("id"),color:c});
...
};for(var a in Smartgraphs){if(Smartgraphs.hasOwnProperty(a)&&Smartgraphs[a]&&Smartgraphs[a].isClass&&a!=="Annotation"&&SC.kindOf(Smartgraphs[a],Smartgraphs.Annotation)){Smartgraphs.Annotation.types.push(Smartgraphs[a]);
Smartgraphs.Annotation.typeNames.push(a)}}Smartgraphs.statechart.initStatechart()

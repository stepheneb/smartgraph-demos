...
}b._annotations[i]=h})});var c=SC.Query.local(Smartgraphs.Variable,"activity={activity}",{activity:d});
var a=Smartgraphs.store.find(c);if(!(a.get("status")&SC.Record.READY)){throw"predefined variable records are not READY!"
}var b=this;a.forEach(function(g){var f=g.get("name");if(b._variables[f]){throw"The activity contains multiple variables named '%@'".fmt(f)
...
})},setupTriggers:function(){},processSubstitutions:function(expressions){if(!expressions){return
}var fmtArgs="";for(var ind in expressions){if(!expressions.hasOwnProperty(ind)){continue
}var expression=expressions[ind];var inspector=this.makeInspector(expression);if(inspector){var value=inspector.inspect();
fmtArgs+=",'"+value+"'"}}fmtArgs=fmtArgs.substring(1);var beforeText=this.get("beforeText");
if(beforeText){this.set("beforeText",eval("beforeText.fmt("+fmtArgs+")"))}var afterText=this.get("afterText");
if(afterText){this.set("afterText",eval("afterText.fmt("+fmtArgs+")"))}},enableSubmission:function(){this.set("canSubmit",YES)
...
sc_require("inspectors/inspector");Smartgraphs.ResponseFieldsInspector=Smartgraphs.Inspector.extend({value:null,fieldIndex:null,configure:function(a){this.set("fieldIndex",a.fieldIndex)
...
this.set("response",response);return response},watch:function(){},stopWatching:function(){}});
(function(g){var e;function h(s,t){for(var r=0;r<s.length;r++){if(e(s[r],t)){return true
}}return false}function k(s,t){for(var r=0;r<s.length;r++){if(!e(s[r],t)){return false
}}return true}function c(r,s){return(e(r[0],s)>e(r[1],s))}function f(r,s){return e(r[0],s)===e(r[1],s)
}function b(r,s){return(e(r,s)||"").strip()}function i(t,v){var s=e(t[0],v);var u=e(t[1],v)||[];
for(var r=0;r<u.length;r++){if(s===u[r]){return true}}return false}function d(r,s){return(e(r,s)||[]).length
}function n(r,s){var t=e(r,s);return(t===0)||!!t}function p(r,s){var t=e(r,s);return t.get&&t.get("x")
}function q(t,y){var u=y.annotations;var r=y.fieldValue;var s=t.axis;var v=t.respectOrder;
if(u&&u.length==2&&r&&s){var x=u[0].get("point");var w=u[1].get("point");var z=w.get(s)-x.get(s);
if(!v){z=Math.abs(z)}return z==r}return false}function o(t,y){var u=y.annotations;
var r=y.fieldValue;if(u&&u.length==2&&r){var x=u[0].get("point");var w=u[1].get("point");
var s=x.get("dataset");if(s!=w.get("dataset")){return false}var A=w.get("x")-x.get("x");
var z=w.get("y")-x.get("y");var v=z/A;return v==r}return false}function m(s,u){if(u.length!=2){return false
}var t=u[0].get("point");var r=u[1].get("point");var v=t.get("dataset");if(v!=r.get("dataset")){return false
}var x=j(v);var y=x.indexOf(t.get("x"));var w=x.indexOf(r.get("x"));return Math.abs(y-w)==1
}function j(s){var t=[];var r=s.get("points");r.forEach(function(u){t.push(u.get("x"))
});return t.sort(function(v,u){return v-u})}function a(r,s){return parseInt(e(r,s),10)
}function l(r,s){return parseFloat(e(r,s),10)}e=function(t,s){if(t==="value"){return s
}if(t===g||t===null||typeof(t)==="string"||typeof(t)==="number"||typeof(t)==="boolean"||t.splice===[].splice){return t
}for(var u in t){if(t.hasOwnProperty(u)){var r=t[u];switch(u){case"literal":return r;
case"or":return h(r,s);case"and":return k(r,s);case"equals":return f(r,s);case"strip":return b(r,s);
case"in":return i(r,s);case"length":return d(r,s);case"gt":return c(r,s);case"notempty":return n(r,s);
case"xvalue":return p(r,s);case"int":return a(r,s);case"float":return l(r,s);case"dataPointsAreAdjacent":return m(r,s);
case"slope":return o(r,s);case"delta":return q(r,s)}console.error('invalid expression operator: "'+u+'"');
...
var j=this.get("store");e.activity=j.readDataHash(this.get("storeKey"));var a=this.get("pages");
e.pages=a.map(function(m){return m.serialize()});var i=a.map(function(m){return m.get("steps").map(function(n){return n.serialize()
})});e.steps=Array.prototype.concat.apply([],i);var g=this.get("axes");e.axes=g.map(function(m){return m.serialize()
});var c=this.get("graphs");e.graphs=c.map(function(m){return m.serialize()});var f=this.get("responseTemplates");
e.responseTemplates=f.map(function(m){return m.serialize()});var b=this.get("datasets");
e.datasets=b.map(function(m){return m.serialize()});var d=b.map(function(m){return m.get("points").map(function(n){return n.serialize()
})});e.datapoints=Array.prototype.concat.apply([],d);var l=this;e.annotations=[];
Smartgraphs.Annotation.typeNames().forEach(function(m){var n=SC.Query.local(Smartgraphs[m],"activity={activity}",{activity:l});
var o=j.find(n);if(o.get("length")>0){e.annotations.push({type:m,records:o.map(function(p){return p.serialize()
})})}});e.variables=[];var h=SC.Query.local(Smartgraphs.Variable,"activity={activity}",{activity:l});
var k=j.find(h);if(k.get("length")>0){e.variables=k.map(function(m){return m.serialize()
...
}return a.create({config:c})}});Smartgraphs.ACTIVITY_STEP_SUBMITTED=SC.State.extend({enterState:function(){var b=Smartgraphs.activityStepController.get("content");

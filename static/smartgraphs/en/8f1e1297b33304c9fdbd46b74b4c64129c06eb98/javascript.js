...
},trace:YES,logDataSource:YES});SC.CONTEXT_MENU_ENABLED=YES;Smartgraphs.activityController=SC.ObjectController.create({canGotoNextPage:NO,cleanup:function(){Smartgraphs.activityPageController.cleanup();
...
}.property("y").cacheable()});Smartgraphs.ArrowView=RaphaelViews.RaphaelView.extend({strokeBinding:".item.color",strokeWidth:2,strokeOpacity:0.5,displayProperties:"point1 point2 label stroke strokeWidth strokeOpacity".w(),renderCallback:function(c,a){var b=c.path(a.d).attr(a);
return b},render:function(c,a){var g,b;var d=this.get("graphView");var f=this.get("item");
if(f.get("isVertical")&&!f.get("isHorizontal")){g=d.coordinatesForPoint(f.get("point2").get("x"),f.get("point1").get("y"))
}else{g=d.coordinatesForPoint(f.get("point1").get("x"),f.get("point1").get("y"))}if(f.get("isHorizontal")){b=d.coordinatesForPoint(f.get("point2").get("x"),f.get("point1").get("y"))
}else{b=d.coordinatesForPoint(f.get("point2").get("x"),f.get("point2").get("y"))}var e=this.arrowPath(g.x,g.y,b.x,b.y,10,15);
var h={d:e,stroke:this.get("stroke"),"stroke-width":this.get("strokeWidth"),"stroke-opacity":this.get("strokeOpacity")};
...
return o}});sc_require("models/annotation");sc_require("models/data_point");sc_require("views/arrow");
Smartgraphs.Arrow=Smartgraphs.Annotation.extend({point1:SC.Record.toOne("Smartgraphs.DataPoint"),point2:SC.Record.toOne("Smartgraphs.DataPoint"),isHorizontal:SC.Record.attr(Boolean,{defaultValue:false}),isVertical:SC.Record.attr(Boolean,{defaultValue:false}),label:SC.Record.attr(String)});
...
return YES},createRiseArrow:function(f,e){var b=Smartgraphs.GraphController.controllerForName[e.graphName];
if(!b){return YES}var g=b.findAnnotationByName(e.firstPoint);var d=b.findAnnotationByName(e.secondPoint);
var c=e.color?e.color:"#000000";var a=Smartgraphs.sessionController.createAnnotation(Smartgraphs.Arrow,e.arrowName,{point1:g.get("point").get("id"),point2:d.get("point").get("id"),color:c,isVertical:YES});
return YES},createRunArrow:function(e,d){var a=Smartgraphs.GraphController.controllerForName[d.graphName];
if(!a){return YES}var g=a.findAnnotationByName(d.firstPoint);var c=a.findAnnotationByName(d.secondPoint);
var b=d.color?d.color:"#000000";var f=Smartgraphs.sessionController.createAnnotation(Smartgraphs.Arrow,d.arrowName,{point1:g.get("point").get("id"),point2:c.get("point").get("id"),color:b,isHorizontal:YES});

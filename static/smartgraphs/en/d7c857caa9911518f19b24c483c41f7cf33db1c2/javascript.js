...
}());Smartgraphs.Activity=SC.Record.extend({url:SC.Record.attr(String),primaryKey:"url",title:SC.Record.attr(String),owner:SC.Record.attr(String),pages:SC.Record.toMany("Smartgraphs.ActivityPage",{inverse:"activity",orderBy:"index"}),context:{}});
...
Smartgraphs.HighlightedSegment.viewClass=Smartgraphs.HighlightedSegmentView;sc_require("models/annotation");
Smartgraphs.IndicatingArrow=Smartgraphs.Annotation.extend({specificX:SC.Record.attr(Number),specificY:SC.Record.attr(Number),annotation:SC.Record.toOne("Smartgraphs.Annotation"),dataPoint:SC.Record.toOne("Smartgraphs.DataPoint"),point_angle:SC.Record.attr(Number,{default_value:335}),length:SC.Record.attr(Number,{default_value:25}),x:function(){if(this.specificX){return this.specificX
}else{if(this.dataPoint){return this.dataPoint.get("x")}else{if(this.annotation){if(this.annotation.get("point")){return this.annotation.get("point").get("x")
}else{if(this.annotation.get("point1")){return this.annotation.get("point1").get("x")
}else{return null}}}else{return null}}}},y:function(){if(this.specificY){return this.specificY
}else{if(this.dataPoint){return this.dataPoint.get("y")}else{if(this.annotation){if(this.annotation.get("point")){return this.annotation.get("point").get("y")
}else{if(this.annotation.get("point1")){return this.annotation.get("point1").get("y")
}else{return null}}}else{return null}}}}});Smartgraphs.LineThroughPointsView=RaphaelViews.RaphaelView.extend({strokeBinding:".item.color",strokeWidth:2,strokeOpacity:0.3,displayProperties:"point1 point2 stroke strokeWidth strokeOpacity".w(),renderCallback:function(c,a){var b=c.path(a.d).attr(a);
return b},render:function(b,a){var c=this.get("graphView");var e=this.get("item");
...
}}if(!b){return YES}var e=b.findAnnotationByName(c.point).get("id");var a=Smartgraphs.sessionController.createAnnotation(Smartgraphs.IndicatingArrow,c.arrowName,{annotation:e,pointAngle:c.angle,color:c.color});
return YES},createIndicatingArrowFromCoordinates:function(c,b){var a=Smartgraphs.sessionController.createAnnotation(Smartgraphs.IndicatingArrow,b.arrowName,{dataPoint:b.point.get("id"),specificX:b.x,specificY:b.y,pointAngle:b.angle,color:b.color});

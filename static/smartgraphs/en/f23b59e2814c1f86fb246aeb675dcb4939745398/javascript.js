...
})();Smartgraphs=SC.Application.create({NAMESPACE:"Smartgraphs",VERSION:"0.1.0",DATA_FORMAT_VERSION:7,rootStore:function(){var a=Smartgraphs.store;
while(a.get("parentStore")){a=a.get("parentStore")}return a}.property(),_nextGuid:1000,getNextGuid:function(){return this._nextGuid++
},trace:YES,logDataSource:YES,showOutline:YES});SC.CONTEXT_MENU_ENABLED=YES;Smartgraphs.activityController=SC.ObjectController.create({canGotoNextPage:NO,isSaving:NO,cleanup:function(){Smartgraphs.activityPageController.cleanup();
...
}},setupPane:function(e,b){var a,d,c=(b.annotations||[]).concat(b.highlightedAnnotations||[]);
this._setAnnotationHighlights(b.annotations,b.highlightedAnnotations);e=Smartgraphs.activityViewController.validPaneFor(e);
if(!e){return}if(b===null){Smartgraphs.statechart.sendAction("hidePane",this,e);return
}switch(b.type){case"graph":if(!b.name){d=Smartgraphs.getNextGuid();a="graph"+d;Smartgraphs.store.createRecord(Smartgraphs.Graph,{url:d,activity:Smartgraphs.activityController.get("id"),name:a,title:b.title,xAxis:b.xAxis,yAxis:b.yAxis,initialDatasets:b.datasets,initialAnnotations:c})
}else{a=b.name}Smartgraphs.statechart.sendAction("showGraph",this,{pane:e,name:a});
return;case"table":Smartgraphs.statechart.sendAction("showTable",this,{pane:e,dataset:b.datasetName||b.dataset,annotations:c});
return;case"image":Smartgraphs.statechart.sendAction("showImage",this,{pane:e,path:b.path,caption:b.caption});
return}},_setAnnotationHighlights:function(b,a){b=b||[];a=a||[];b.forEach(function(d){var c=Smartgraphs.activityObjectsController.findAnnotation(d);
if(c){c.set("isHighlighted",NO)}});a.forEach(function(d){var c=Smartgraphs.activityObjectsController.findAnnotation(d);
if(c){c.set("isHighlighted",YES)}})},setContextVars:function(a){if(!a){return}a.forEach(function(b){Smartgraphs.activityPageController.setInContext(b.name,Smartgraphs.evaluator.evaluate(b.value))
})},startTools:function(){var a=this.get("tools");a.forEach(function(b){switch(b.type){case"dataPointTagging":Smartgraphs.statechart.sendAction("startInteractiveSelection",null,{annotationName:b.tagName,datasetName:b.dataset});
...
}return a.create({config:b.config})},panesJsonDidChange:function(){var b=this.get("panesJson");
if(b){var c;try{c=SC.json.decode(b);this.set("jsonEditingFeedback","")}catch(d){this.set("jsonEditingFeedback","JSON parsing error.");
return}var a=this.get("panes");if(c!=a){this.set("panes",c)}}}.observes("panesJson"),panesDidChange:function(){var b=this.get("panesJson");
var a=JSON.stringify(this.get("panes"),null,2);if(a!=b){this.set("panesJson",a)}}.observes("panes")});
...
return NO},retrieveRecord:function(b,d){var f=b.recordTypeFor(d);var e=b.idFor(d);
this.log("CouchDataSource.retrieveRecord()");this.log("  Record type requested = %s",f.toString());
...
return YES}this.log("returning NO from retrieveRecord");return NO},didRetrieveActivity:function(d,c,e){if(SC.ok(d)){var a=d.get("body");
this.log("retrieved response.body = ",a);if(a&&a.rows&&a.rows.length===1&&a.rows[0].value){var f=a.rows[0].value;
this.log("doc = ",f);this.log("doc.activity = ",f.activity);this._ids[e]=f._id;this._revs[e]=f._rev;
var b=this;[["ActivityPage","pages"],["ActivityStep","steps"],["Unit","units"],["Axis","axes"],["Graph","graphs"],["ResponseTemplate","responseTemplates"],["DataPoint","datapoints"],["Dataset","datasets"]].forEach(function(g){b.loadRecordsFromArray(c,Smartgraphs[g[0]],f[g[1]])
...
return this},dependsOn:function(){this.deps=Array.prototype.splice.call(arguments,0);
...
Smartgraphs.Annotation.typeNames().forEach(function(n){var q=Smartgraphs[n],o=SC.Query.local(q,"activity={activity}",{activity:m}),p=k.find(o).filter(function(r){return r.constructor===q
});if(p.get("length")>0){e.annotations.push({type:n,records:p.map(function(r){return r.serialize()
...
}return c}}());sc_require("mixins/arrow_drawing");Smartgraphs.ArrowView=RaphaelViews.RaphaelView.extend(Smartgraphs.ArrowDrawing,{canShowInTable:NO,itemColorBinding:".item.color",isHighlightedBinding:".item.isHighlighted",strokeWidth:2,strokeOpacity:0.9,stroke:function(){return this.get("isHighlighted")?"#ff0000":this.get("itemColor")
}.property("isHighlighted","itemColor"),displayProperties:"item.x1 item.y1 item.x2 item.y2 label stroke isHighlighted strokeWidth strokeOpacity".w(),renderCallback:function(g,a,d,f,c){var e=f&&f.x||0;
var b=f&&f.y||0;return g.set().push(g.path(a.d).attr(a),g.text(e,b,d||"").attr({fill:a.stroke,"font-size":15,"stroke-width":1}).rotate(c||0,true))
},render:function(c,h){var g=this.get("graphView"),d=this.get("item"),i=d&&d.get("label")||"",p=d&&d.get("x1")||0,e=d&&d.get("y1")||0,n=d&&d.get("x2")||0,b=d&&d.get("y2")||0,o=g.coordinatesForPoint(p,e),m=g.coordinatesForPoint(n,b),k=(p===n)&&(e===b),r={d:k?"":this.arrowPath(o.x,o.y,m.x,m.y,10,15),stroke:this.get("stroke"),"stroke-width":this.get("strokeWidth"),"stroke-opacity":this.get("strokeOpacity")},f,l,a,j,q;
if(p===n){f=g.coordinatesForPoint(p,(e+b)/2);f.x-=12;l=-90}else{f=g.coordinatesForPoint((p+n)/2,e);
f.y-=12;l=0}if(h){c.callback(this,this.renderCallback,r,i,f,l)}else{a=c.raphael();
j=a.items[0];q=a.items[1];j.attr(r);q.attr({fill:this.get("stroke")})}}});sc_require("models/annotation");
sc_require("views/arrow");Smartgraphs.Arrow=Smartgraphs.Annotation.extend({x1:SC.Record.attr(Number),y1:SC.Record.attr(Number),x2:SC.Record.attr(Number),y2:SC.Record.attr(Number),label:SC.Record.attr(String),p1:SC.Record.toOne("Smartgraphs.DataPoint"),p2:SC.Record.toOne("Smartgraphs.DataPoint"),p1Tag:SC.Record.toOne("Smartgraphs.HighlightedPoint"),p2Tag:SC.Record.toOne("Smartgraphs.HighlightedPoint"),_setPointsFromTags:function(){var b=this.getPath("p1Tag.point"),a=this.getPath("p2Tag.point");
if(b){this.setIfChanged("p1",b)}if(a){this.setIfChanged("p2",a)}}.observes("*p1Tag.point","*p2Tag.point"),_startAndEndDidChange:function(){var c=this.get("p1"),b=this.get("p2"),a;
if(!c||!b){return}a=this.calculateCoordinatesFromPoints(this.get("p1"),this.get("p2"));
this.setIfChanged("x1",a.x1);this.setIfChanged("y1",a.y1);this.setIfChanged("x2",a.x2);
this.setIfChanged("y2",a.y2)}.observes("*p1.x","*p1.y","*p2.x","*p2.y"),calculateCoordinatesFromPoints:function(b,a){return{x1:b.get("x"),y1:b.get("y"),x2:a.get("x"),y2:a.get("y")}
}});Smartgraphs.Arrow.viewClass=Smartgraphs.ArrowView;Smartgraphs.Arrow.NO_ARROW_HEADS=0;
Smartgraphs.Arrow.ARROW_HEAD_AT_START=1;Smartgraphs.Arrow.ARROW_HEAD_AT_END=2;Smartgraphs.Arrow.ARROW_HEAD_AT_START_AND_END=3;
...
sc_require("mixins/arrow_drawing");Smartgraphs.BracketArcView=RaphaelViews.RaphaelView.extend({canShowInTable:YES,itemColorBinding:".item.color",isHighlightedBinding:".item.isHighlighted",strokeWidth:2,strokeOpacity:0.9,stroke:function(){return this.get("isHighlighted")?"#ff0000":this.get("itemColor")
}.property("isHighlighted","itemColor"),displayProperties:"bracket.item1Index bracket.item2Index label stroke isHighlighted strokeWidth strokeOpacity".w(),renderCallback:function(f,d,c,e,b,a){return f.set().push(f.path(d.d).attr(d),f.text(e||0,b||0,c||"").attr({fill:d.stroke,"font-size":15}).rotate(a||0,true))
},render:function(d,e){var u=this.get("item"),n=u.get("item1Index"),m=u.get("item2Index"),q=u.get("isLeftOfColumn"),l=this.get("tableView"),r=l.coordinatesForIndex(n),p=l.coordinatesForIndex(m),t=q?r.left-10:r.right+10,s=(r.top+r.bottom)/2,c=t,b=(p.top+p.bottom)/2,f=u.get("label")||"",o=q?-90:90,h=q?t-10:t+10,g=(s+b)/2,j=this.figurePath(t,s,c,b,q),k={d:j,stroke:this.get("stroke"),"stroke-width":this.get("strokeWidth"),"stroke-opacity":this.get("strokeOpacity")},a,i,v;
if(e){d.callback(this,this.renderCallback,k,f,h,g,o)}else{a=d.raphael();i=a.items[0];
v=a.items[1];i.attr(k);v.attr({fill:this.get("stroke")})}},figurePath:function(v,u,d,a,t){var w=25,g=6,p,o,k,m,j,i,h,e,b,n,l,s,r,f,c,q;
if(t){k=Math.atan2((a-u),(d-v))-(Math.PI/2);p={x:v-40,y:u};o={x:d-40,y:a};if(u<a){m=k+(0.5*w)*Math.PI/180;
j=k-(1.5*w)*Math.PI/180;i=k+(1.5*w)*Math.PI/180;h=k-(0.5*w)*Math.PI/180}else{m=k+(1.5*w)*Math.PI/180;
j=k-(0.5*w)*Math.PI/180;i=k+(0.5*w)*Math.PI/180;h=k-(1.5*w)*Math.PI/180}}else{k=Math.atan2((a-u),(d-v))+(Math.PI/2);
p={x:v+40,y:u};o={x:d+40,y:a};if(u<a){m=k+(1.5*w)*Math.PI/180;j=k-(0.5*w)*Math.PI/180;
i=k+(0.5*w)*Math.PI/180;h=k-(1.5*w)*Math.PI/180}else{m=k+(0.5*w)*Math.PI/180;j=k-(1.5*w)*Math.PI/180;
i=k+(1.5*w)*Math.PI/180;h=k-(0.5*w)*Math.PI/180}}if(u<a){e=v-g*Math.cos(m);b=u-g*Math.sin(m);
n=v-g*Math.cos(j);l=u-g*Math.sin(j);s=d-g*Math.cos(i);r=a-g*Math.sin(i);f=d-g*Math.cos(h);
c=a-g*Math.sin(h)}else{e=v+g*Math.cos(m);b=u+g*Math.sin(m);n=v+g*Math.cos(j);l=u+g*Math.sin(j);
s=d+g*Math.cos(i);r=a+g*Math.sin(i);f=d+g*Math.cos(h);c=a+g*Math.sin(h)}q="M "+v+" "+u+"C "+p.x+" "+p.y+" "+o.x+" "+o.y+" "+d+" "+a+"L "+s+" "+r+"L "+f+" "+c+"L "+d+" "+a;
return q}});sc_require("models/annotation");sc_require("views/bracket_arc");Smartgraphs.BracketArc=Smartgraphs.Annotation.extend({item1Index:SC.Record.attr(Number),item2Index:SC.Record.attr(Number),isLeftOfColumn:SC.Record.attr(Boolean),p1:SC.Record.toOne("Smartgraphs.DataPoint"),p2:SC.Record.toOne("Smartgraphs.DataPoint"),p1Tag:SC.Record.toOne("Smartgraphs.HighlightedPoint"),p2Tag:SC.Record.toOne("Smartgraphs.HighlightedPoint"),_setPointsFromTags:function(){var b=this.getPath("p1Tag.point"),a=this.getPath("p2Tag.point");
if(b){this.setIfChanged("p1",b)}if(a){this.setIfChanged("p2",a)}}.observes("*p1Tag.point","*p2Tag.point"),_pointIndicesDidChange:function(){var c=this.get("p1"),a=this.get("p2"),b;
if(!c||!a){return}if(SC.none(c.get("dataset"))||(c.get("dataset")!==a.get("dataset"))){return
}b=this.calculateIndicesFromPoints(c,a);this.setIfChanged("item1Index",b.item1Index);
this.setIfChanged("item2Index",b.item2Index)}.observes("*p1.dataset.points.[]","p1","p2"),calculateIndicesFromPoints:function(c,b){var a=c.getPath("dataset.points")||[];
return{item1Index:a.indexOf(c),item2Index:a.indexOf(b)}}});Smartgraphs.BracketArc.viewClass=Smartgraphs.BracketArcView;
Smartgraphs.DataPoint=SC.Record.extend({x:SC.Record.attr(Number),y:SC.Record.attr(Number),dataset:SC.Record.toOne("Smartgraphs.Dataset",{inverse:"points",isMaster:YES,aggregate:YES}),xRounded:function(){return Math.round(this.get("x")*100)/100
}.property("x").cacheable(),yRounded:function(){return Math.round(this.get("y")*100)/100
}.property("y").cacheable()});SC.RecordAttribute.registerTransform(Smartgraphs.DataPoint,{to:function(d,c,f,e){var b,a;
if(SC.none(d)||(d==="")){return null}b=Smartgraphs.activityObjectsController.findAnnotation(d);
a=b&&b.get("point");if(a){return a}a=Smartgraphs.store.find(f,d);return a||null},from:function(a){return a?a.get("id"):null
}});Smartgraphs.AnnotatableItemView={initMixin:function(){this._aiv_baseValues={};
...
sc_require("models/arrow");Smartgraphs.RiseArrow=Smartgraphs.Arrow.extend({calculateCoordinatesFromPoints:function(e,d){var a=e.get("x"),b=d.get("x"),c=(a<b)?{left:e,right:d}:{left:d,right:e};
return{x1:c.left.get("x"),y1:c.left.get("y"),x2:c.left.get("x"),y2:c.right.get("y")}
}});Smartgraphs.RiseBracket=Smartgraphs.BracketArc.extend({isLeftOfColumn:NO,calculateIndicesFromPoints:function(e,d){var c=e.getPath("dataset.points")||[],b=c.indexOf(e),a=c.indexOf(d);
return{item1Index:Math.min(b,a),item2Index:Math.max(b,a)}}});sc_require("models/arrow");
Smartgraphs.RunArrow=Smartgraphs.Arrow.extend({calculateCoordinatesFromPoints:function(e,d){var a=e.get("x"),b=d.get("x"),c=(a<b)?{left:e,right:d}:{left:d,right:e};
return{x1:c.left.get("x"),y1:c.right.get("y"),x2:c.right.get("x"),y2:c.right.get("y")}
}});Smartgraphs.RunBracket=Smartgraphs.BracketArc.extend({isLeftOfColumn:YES,calculateIndicesFromPoints:function(e,d){var c=e.getPath("dataset.points")||[],b=c.indexOf(e),a=c.indexOf(d);
return{item1Index:Math.min(b,a),item2Index:Math.max(b,a)}}});Smartgraphs.Session=SC.Record.extend({user:SC.Record.toOne("Smartgraphs.User",{inverse:"session",isMaster:YES})});
...
return YES},createRiseBracket:function(a,f){var e=Smartgraphs.activityObjectsController.findAnnotation(f.point1),d=Smartgraphs.activityObjectsController.findAnnotation(f.point2),i=e.get("point"),g=d.get("point"),c=i.get("dataset"),h=c.get("points"),j=h.indexOf(i),b=h.indexOf(g);
if(i.get("dataset")!==g.get("dataset")){return YES}Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.BracketArc,f.bracketName,{item1Index:Math.min(j,b),item2Index:Math.max(j,b),isLeftOfColumn:NO,color:f.color||"#cc0000",isHighlighted:f.isHighlighted||NO,label:f.label});
return YES},createRunBracket:function(a,f){var e=Smartgraphs.activityObjectsController.findAnnotation(f.point1),d=Smartgraphs.activityObjectsController.findAnnotation(f.point2),i=e.get("point"),g=d.get("point"),c=i.get("dataset"),h=c.get("points"),j=h.indexOf(i),b=h.indexOf(g);
if(i.get("dataset")!==g.get("dataset")){return YES}Smartgraphs.activityObjectsController.createAnnotation(Smartgraphs.BracketArc,f.bracketName,{item1Index:Math.min(j,b),item2Index:Math.max(j,b),isLeftOfColumn:YES,color:f.color||"#cc0000",isHighlighted:f.isHighlighted||NO,label:f.label});
...
}},_addViewForItemToBackdrop:function(c,d){var b=SC.guidFor(c.constructor);var a=c.constructor.viewClass.design({tableView:this,item:c,itemType:d}).create();
if(a.get("canShowInTable")){this.getPath("tableColumnView.scrollView.contentView.backdropView.annotationsHolder").appendChild(a);
...
this.$().addClass("hovered")},mouseExited:function(){this.$().removeClass("hovered");
this.adjust("height",this.get("originalHeight"))}}),activityStepView:SC.View.design({useStaticLayout:YES,isVisibleBinding:"Smartgraphs.activityStepController.dialogTextHasContent",childViews:"beforeTextView afterTextView".w(),classNames:"dialog-text".w(),beforeTextView:SC.StaticContentView.design({contentBinding:"Smartgraphs.activityStepController.beforeText",isVisibleBinding:SC.Binding.bool("Smartgraphs.activityStepController.beforeText")}),afterTextView:SC.StaticContentView.design({contentBinding:"Smartgraphs.activityStepController.afterText",isVisibleBinding:SC.Binding.bool("Smartgraphs.activityStepController.afterText")})})})})}),dataWrapper:SC.View.design({layout:{right:0,width:0.55},childViews:"dataView".w(),dataView:SC.View.design({layout:{top:4,right:4,bottom:4,left:4},childViews:"jsonEditorFeedback panesAsJsonView".w(),classNames:"json-editor smartgraph-pane".w(),jsonEditorFeedback:SC.LabelView.design({classNames:"json-editor-feedback".w(),valueBinding:"Smartgraphs.activityStepController.jsonEditingFeedback",escapeHTML:NO,showEditor:NO,isVisibleBinding:SC.Binding.bool("Smartgraphs.activityStepController.jsonEditingFeedback")}),panesAsJsonView:SC.LabelView.design({layout:{top:10,right:10,bottom:10,left:10},valueBinding:"Smartgraphs.activityStepController.panesJson",escapeHTML:YES,isEditable:YES,isInlineEditorMultiline:YES,classNames:"json-editor-code".w(),showEditor:NO,showEditorDidChange:function(){if(this.get("showEditor")){this.beginEditing()
}}.observes("showEditor"),beginEditing:function(){arguments.callee.base.apply(this,arguments)
},inlineEditorDidEndEditing:function(){arguments.callee.base.apply(this,arguments);
this.set("showEditor",NO)},mouseEntered:function(){this.$().addClass("hovered")},mouseExited:function(){this.$().removeClass("hovered")
},mouseDown:function(){this.beginEditing()}})})})})});Smartgraphs.authorPage=Smartgraphs.authorPageDef.design();
Smartgraphs.main=function main(){Smartgraphs.dataSource=SC.CascadeDataSource.create({dataSources:"couch fixtures".w(),couch:Smartgraphs.CouchDataSource.create(),fixtures:SC.FixturesDataSource.create()});

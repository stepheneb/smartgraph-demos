...
this._annotations={};this._variables={};var d=Smartgraphs.activityController.get("activityRecordInCurrentStore");
...
}b._datasets[f]=g});Smartgraphs.Annotation.types().forEach(function(f){c=SC.Query.local(f,"activity={activity}",{activity:Smartgraphs.activityController.get("activityRecordInCurrentStore")});
...
if(c){a.removeObject(c)}},findDatasetByName:function(a){return this.findObjectByNameIn(a,this.get("datasetList"))
...
var j=this.get("item").get("label");var i=d.coordinatesForPoint(e.start.x,e.start.y);
var b=d.coordinatesForPoint(e.end.x,e.end.y);var f,h;if(j&&this.get("item").get("isHorizontal")){f=d.coordinatesForPoint(((e.start.x+e.end.x)/2),(e.start.y+0.3));
h=0}else{if(j&&this.get("item").get("isVertical")){f=d.coordinatesForPoint((e.start.x-0.15),((e.start.y+e.end.y)/2));
h=-90}}var g;if((i.x!==b.x)||(i.y!==b.y)){g=this.arrowPath(i.x,i.y,b.x,b.y,10,15)
}else{g="";j=""}var k={d:g,stroke:this.get("stroke"),"stroke-width":this.get("strokeWidth"),"stroke-opacity":this.get("strokeOpacity"),label:j,labelX:f.x,labelY:f.y,rotate:h};
if(a){c.callback(this,this.renderCallback,k)}else{var l=c.raphael();l.attr(k)}},getStartAndEnd:function(d){var k=d.get("point1"),j=d.get("point2");
...
if(b===c&&c.get("isFinalStep")){var a=this;this.invokeLast(function(){a.gotoState("ACTIVITY_PAGE_DONE")
})}},exitState:function(){Smartgraphs.activityStepController.cleanup()},gotoStep:function(b,a){var c=Smartgraphs.get("rootStore").find(Smartgraphs.ActivityStep,a.stepId);
...
Smartgraphs.store=SC.Store.create().from(Smartgraphs.dataSource);for(var a in Smartgraphs){if(Smartgraphs.hasOwnProperty(a)&&Smartgraphs[a]&&Smartgraphs[a].isClass&&Smartgraphs[a].FIXTURES){Smartgraphs.store.find(Smartgraphs[a])

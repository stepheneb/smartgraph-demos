...
}}});Smartgraphs.activityStepController=SC.ObjectController.create({registeredTriggers:[],begin:function(){this.registerTriggerResponses();
...
}});Smartgraphs.GraphController=SC.ObjectController.extend(SC.Responder,{seriesList:null,openGraph:function(a){var e=Smartgraphs.store.find(Smartgraphs.Graph,a);
if(!e){e=Smartgraphs.store.createRecord(Smartgraphs.Graph,{id:a});Smartgraphs.store.commitRecords()
}this.set("content",e);this.set("seriesList",[]);var b=this.get("initialSeries");
...
}}},removeAllSeries:function(){},view:function(){return Smartgraphs.getPath(this.get("viewPath"))
...
},clear:function(){this.set("seriesList",[]);this.set("content",[])}});sc_require("controllers/graph");
Smartgraphs.firstGraphController=Smartgraphs.GraphController.create({viewPath:"activityPage.firstGraphView"});
...
}this.addOrUpdatePredictionPoint(a,d);return YES},addSensorPoint:function(b,c){var a=Smartgraphs.store.createRecord(Smartgraphs.DataPoint,{x:b,y:c,guid:Smartgraphs.getNextGuid()});
...
return b},copyExampleSeries:function(m,c){var h=SC.Query.local(Smartgraphs.DataSeries,"isExample=YES AND name={seriesName}",{seriesName:m});
...
Smartgraphs.mockResponses["/backend/activity/1/page/3/step/2/commands"]=[{url:"/backend/activity/1/page/3/step/2/response/1/command/1/wait-for-valid-response",command:"/backend/command/11/wait-for-valid-response",triggerResponse:"/backend/activity/1/page/3/step/2/response/1/step-beginning",index:1,literalArgs:{},substitutedArgs:{}}];
...
commandList.push(command);command={url:"/backend/activity/2/page/1/step/1/response/1/command/3/predict",command:"/backend/command/7/enable-prediction-input",triggerResponse:"/backend/activity/2/page/1/step/1/response/1/step-beginning",index:3,literalArgs:{pane:"single",seriesName:"prediction-away",xMin:0,xMax:15},substitutedArgs:{}};
...
if(a&SC.Record.READY){return this.checkSubordinateResources()}else{if(a&SC.Record.ERROR){if(this.resourceLoadingError){this.resourceLoadingError()
}this.cleanupLoading();return YES}return NO}},checkSubordinateResources:function(){var d,b=this.get("subordinateResources");
for(var a=0,c=b.get("length");a<c;a++){d=b[a];if(d.record===null){d.record=d.load(this);
this.watch(d.record)}}if(this.subordinateResourcesAreReady()){this.resourcesDidLoad();
this.cleanupLoading();return YES}if(this.subordinateResourcesHaveErrors()){if(this.resourceLoadingError){this.resourceLoadingError()
}this.cleanupLoading();return YES}return NO},cleanupLoading:function(){this._watchedRecords.forEach(function(a){a.removeObserver("status",this,this.checkResourceStatuses)
...
Smartgraphs.makeFirstResponder(Smartgraphs.ACTIVITY_LOADING_PAGE)}});sc_require("states/activity");
Smartgraphs.ACTIVITY_STEP=SC.Responder.create({nextResponder:Smartgraphs.ACTIVITY,didBecomeFirstResponder:function(){Smartgraphs.activityStepController.set("submitButtonShouldBeEnabled",YES);
Smartgraphs.activityStepController.invokeLater(Smartgraphs.activityStepController.begin)
},_graphControllerFor:function(a){if(!Smartgraphs.activityViewController.get("paneIsSplit")||a==="top"){return Smartgraphs.firstGraphController
...
}if(a==="bottom"){return Smartgraphs.getPath("activityPage.firstGraphView")}},finishActivityStep:function(){Smartgraphs.makeFirstResponder(Smartgraphs.ACTIVITY_STEP_DONE);
...
return YES},removeAllSeries:function(b,a){return NO},selectDataSeries:function(d,b){var a=this._graphControllerFor(b.pane);
var c=a.findSeries(b.seriesName);if(c.get("isExample")===NO){Smartgraphs.selectedSeriesController.set("content",c)
}return YES},enablePredictionInput:function(c,b){Smartgraphs.sendAction("createSeriesOnGraph",this,{pane:b.pane,seriesName:b.seriesName});
Smartgraphs.sendAction("selectDataSeries",this,{pane:b.pane,seriesName:b.seriesName});
Smartgraphs.selectedSeriesController.set("xMin",b.xMin);Smartgraphs.selectedSeriesController.set("xMax",b.xMax);
var a=this._graphControllerFor(b.pane);Smartgraphs.inputGraphController=this._graphControllerFor(b.pane);
Smartgraphs.GRAPH_INPUT.set("nextResponder",Smartgraphs.get("firstResponder"));Smartgraphs.makeFirstResponder(Smartgraphs.GRAPH_INPUT);
return YES},enableSensorInput:function(b,a){Smartgraphs.sendAction("createSeriesOnGraph",this,{pane:a.pane,seriesName:a.seriesName});
Smartgraphs.sendAction("selectDataSeries",this,{pane:a.pane,seriesName:a.seriesName});
Smartgraphs.SENSOR.set("nextResponder",Smartgraphs.get("firstResponder"));Smartgraphs.makeFirstResponder(Smartgraphs.SENSOR);
return YES}});sc_require("states/activity");Smartgraphs.ACTIVITY_STEP_DONE=SC.Responder.create({nextResponder:Smartgraphs.ACTIVITY,didBecomeFirstResponder:function(){var a=Smartgraphs.activityStepController.get("isLastStep");
...
}});sc_require("states/activity_step");Smartgraphs.ACTIVITY_STEP_SUBMIT=SC.Responder.create({nextResponder:Smartgraphs.ACTIVITY_STEP,didBecomeFirstResponder:function(){Smartgraphs.activityStepController.set("submitButtonShouldBeEnabled",YES)
...
return YES}});sc_require("states/activity_step");Smartgraphs.ACTIVITY_STEP_WAITING=SC.Responder.create({nextResponder:Smartgraphs.ACTIVITY_STEP,didBecomeFirstResponder:function(){Smartgraphs.activityStepController.set("submitButtonShouldBeEnabled",NO)
},enableSubmission:function(){Smartgraphs.makeFirstResponder(Smartgraphs.ACTIVITY_STEP_SUBMIT);
return YES},finishActivityStep:function(){return YES}});sc_require("states/ready");
Smartgraphs.ERROR_LOADING_ACTIVITY=SC.Responder.create({nextResponder:Smartgraphs.READY,didBecomeFirstResponder:function(){Smartgraphs.appWindowController.showActivityLoadingErrorView()
}});Smartgraphs.GRAPH_INPUT=SC.Responder.create({nextResponder:null,didBecomeFirstResponder:function(){Smartgraphs.inputGraphController.startRoutingInputEvents()
},willLoseFirstResponder:function(){Smartgraphs.inputGraphController.stopRoutingInputEvents();
Smartgraphs.selectedSeriesController.set("content",null)},resignFirstResponder:function(a){if(Smartgraphs.get("firstResponder")===this){Smartgraphs.makeFirstResponder(this.get("nextResponder"),a)
...
return YES},endPredictionGraphInput:function(){this.resignFirstResponder()}});sc_require("states/ready");
...
Smartgraphs.makeFirstResponder(Smartgraphs.READY)}});Smartgraphs.SENSOR=SC.Responder.create({nextResponder:null,didBecomeFirstResponder:function(){Smartgraphs.appletPage.get("appletPane").append()
...
Smartgraphs.SENSOR_ERROR=SC.Responder.create({nextResponder:Smartgraphs.SENSOR,didBecomeFirstResponder:function(){},willLoseFirstResponder:function(){}});
...
}}});Smartgraphs.GraphView=SC.View.extend({axesBinding:"*graph.axes",seriesListBinding:"*graph.seriesList",padding:{top:20,right:20,bottom:40,left:60},childViews:"graphCanvasView".w(),init:function(){this._seriesViewsById={};
...
var m=i/f;var c=h/e;return{x:(l-k.left)/m,y:(k.top+h-j)/c}},_seriesListDidChange:function(){var d=this.get("seriesList");
var e,a;var b={};var h=[],c=[];for(var f=0,j=d.get("length");f<j;f++){e=d.objectAt(f);
a=e.get("id");b[a]=e;if(!this._seriesViewsById.hasOwnProperty(a)){this._addViewForSeries(e)

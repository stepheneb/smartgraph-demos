...
}});Smartgraphs.activityPagesController=SC.ArrayController.create({allowsMultipleSelection:NO,shouldShowStepsInOutline:YES,currentPageDidChange:function(){this.selectObject(Smartgraphs.activityPageController.get("content"))
...
this.forEach(function(b){b.set("pageNumber",a++)})}.observes("[]"),outline:function(){var a=!this.get("shouldShowStepsInOutline");
return SC.Object.create({title:"toplevel",treeItemIsExpanded:YES,pages:this.map(function(b){return b
}),treeItemChildren:this.map(function(b){var c=1;return SC.Object.create({title:b.get("name")||"Page %@".fmt(b.get("pageNumber")+1),treeItemIsExpanded:YES,page:b,steps:a?undefined:b.get("steps"),treeItemChildren:a?undefined:b.get("steps").map(function(d){return SC.Object.create({title:"Step %@".fmt(c++),step:d,treeItemIsExpanded:YES,treeItemChildren:null})
})})})})}.property("[]","shouldShowStepsInOutline").cacheable()});Smartgraphs.activityStepController=SC.ObjectController.create({canSubmit:NO,showSubmitButton:NO,submissibilityInspectorInstance:null,cleanup:function(){var a=this.get("submissibilityInspectorInstance");
...
Smartgraphs.sendAction("enableSubmission");this.executeCommands(this.get("startCommands"));
this.setupTriggers();if(this.get("shouldFinishImmediately")){Smartgraphs.sendAction("submitStep")
}else{Smartgraphs.sendAction("waitForResponse")}},setupPanes:function(){Smartgraphs.sendAction("setPaneConfig",this,this.get("paneConfig"));
...
return;case"table":Smartgraphs.sendAction("showTable",this,{pane:b,datasetName:a.datasetName});
return;case"image":Smartgraphs.sendAction("showImage",this,{pane:b,path:a.path});
return}},executeCommands:function(a){if(!a){return}var b=this;a.forEach(function(c){Smartgraphs.sendAction(c.action,b,c.literalArgs)
...
Smartgraphs.sendAction("disableSubmission");b.addObserver("value",this,this.checkSubmissibility);
...
if(b&&!a){Smartgraphs.sendAction("enableSubmission")}else{if(a&&!b){Smartgraphs.sendAction("disableSubmission")
...
for(var c=0;c<b.length;c++){d=b.objectAt(c);if(Smartgraphs.evaluate(d.criterion,e)){Smartgraphs.sendAction("gotoStep",this,{stepId:d.step});
...
if(a.kindOf(Smartgraphs.HighlightedPoint)){a.addObserver("point",this,"updateDataPoints")
...
}.property("y").cacheable()});Smartgraphs.ArrowView=RaphaelViews.RaphaelView.extend({strokeBinding:".item.color",isHighlightedBinding:".item.isHighlighted",strokeWidth:function(){return this.get("isHighlighted")?3:2
...
if(a){c.callback(this,this.renderCallback,h)}else{var i=c.raphael();i.attr(h)}},arrowPath:function(b,a,j,i,k,f){var e=Math.atan2((i-a),(j-b));
...
return o},getStartAndEnd:function(d){var k=d.get("point1"),j=d.get("point2");var c=k.get("x"),h=k.get("y"),a=j.get("x"),g=j.get("y");
var i,f,b,e;if(d.get("isHorizontal")){f=d.get("isClockwise")?g:h;b={x:c,y:f};e={x:a,y:f}
}else{if(d.get("isVertical")){i=d.get("isClockwise")?c:a;b={x:i,y:h};e={x:i,y:g}}else{b={x:c,y:h};
e={x:a,y:g}}}return{start:b,end:e}}});sc_require("models/annotation");sc_require("models/data_point");
sc_require("views/arrow");Smartgraphs.Arrow=Smartgraphs.Annotation.extend({point1:SC.Record.toOne("Smartgraphs.DataPoint"),point2:SC.Record.toOne("Smartgraphs.DataPoint"),isHorizontal:SC.Record.attr(Boolean,{defaultValue:false}),isVertical:SC.Record.attr(Boolean,{defaultValue:false}),isClockwise:SC.Record.attr(Boolean,{defaultValue:false}),label:SC.Record.attr(String)});
...
},willLoseFirstResponder:function(){},route:function(a){if(a.activityId){Smartgraphs.sendAction("openActivity",this,{id:a.activityId})
...
},willLoseFirstResponder:function(){Smartgraphs.activityController.cleanup()},openAuthorView:function(){Smartgraphs.makeFirstResponder(Smartgraphs.AUTHOR)
},openActivity:function(){Smartgraphs.LOADING_ACTIVITY.set("openAuthorViewAfterLoading",NO);
...
}else{a.set("isHighlighted",YES)}return YES}});sc_require("states/activity");Smartgraphs.ACTIVITY_DONE=SC.Responder.create({nextResponder:Smartgraphs.ACTIVITY});
...
return YES},submitStep:function(){if(Smartgraphs.activityStepController.get("canSubmit")){Smartgraphs.makeFirstResponder(Smartgraphs.ACTIVITY_STEP_SUBMITTED)
...
Smartgraphs.INTERACTIVE_SELECTION.set("dataset",e);Smartgraphs.makeFirstResponder(Smartgraphs.INTERACTIVE_SELECTION);
...
return YES}});sc_require("states/ready");Smartgraphs.AUTHOR=SC.Responder.create({nextResponder:Smartgraphs.READY,didBecomeFirstResponder:function(){Smartgraphs.appWindowController.showAuthorView();
...
Smartgraphs.activityViewController.set("enableBackAndForward",YES)},willLoseFirstResponder:function(){Smartgraphs.activityPagesController.set("shouldShowStepsInOutline",YES);
...
Smartgraphs.activityViewController.set("enableBackAndForward",NO)},openActivity:function(){Smartgraphs.LOADING_ACTIVITY.set("openAuthorViewAfterLoading",YES);
return NO},runActivity:function(){Smartgraphs.LOADING_ACTIVITY.set("openAuthorViewAfterLoading",NO);
Smartgraphs.makeFirstResponder(Smartgraphs.LOADING_ACTIVITY);return YES},gotoNextPage:function(){Smartgraphs.activityPagesController.selectNextPage()
},gotoPreviousPage:function(){Smartgraphs.activityPagesController.selectPreviousPage()
}});sc_require("states/ready");Smartgraphs.ERROR_LOADING_ACTIVITY=SC.Responder.create({nextResponder:Smartgraphs.READY,didBecomeFirstResponder:function(){Smartgraphs.appWindowController.showErrorLoadingActivityView()
...
}});Smartgraphs.INTERACTIVE_SELECTION=SC.Responder.create({nextResponder:Smartgraphs.ACTIVITY_STEP,annotation:null,dataset:null,didBecomeFirstResponder:function(){Smartgraphs.sendAction("disableSubmission");
var a=this.get("dataset");this._oldIsSelectable=a.get("isSelectable");a.set("isSelectable",NO)
},willLoseFirstResponder:function(){var a=this.get("dataset");a.set("isSelectable",this._oldIsSelectable);
this.set("dataset",null);this.set("annotation",null)},dataPointSelected:function(b){var c=this.get("dataset");
var a=b.get("content");if(c&&a.get("dataset")===c){this.setPath("annotation.point",a);
Smartgraphs.sendAction("enableSubmission")}},startInteractiveSelection:function(){return YES
}});sc_require("states/ready");sc_require("states/mixins/resource_loader");Smartgraphs.LOADING_ACTIVITY=SC.Responder.create(Smartgraphs.ResourceLoader,{nextResponder:Smartgraphs.READY,openAuthorViewAfterLoading:NO,masterResource:{load:function(){return Smartgraphs.activityController.get("content")
}},subordinateResources:[{load:function(){return Smartgraphs.store.find(Smartgraphs.activityController.get("pagesQuery"))
}}],didBecomeFirstResponder:function(){if(this.get("openAuthorViewAfterLoading")){Smartgraphs.toolbarController.showRunButton()
}else{Smartgraphs.toolbarController.showEditButton()}if(this.loadResources()){return
}Smartgraphs.appWindowController.showActivityLoadingView()},willLoseFirstResponder:function(){this.cancelLoading()
...
}Smartgraphs.invokeLater(Smartgraphs.resetFirstResponder);return NO},openAuthorView:function(){this.set("openAuthorViewAfterLoading",YES);
return YES},runActivity:function(){this.set("openAuthorViewAfterLoading",NO);return YES
}});Smartgraphs.LOGIN=SC.Responder.create({nextResponder:null,didBecomeFirstResponder:function(){Smartgraphs.userController.set("content",Smartgraphs.store.find(Smartgraphs.User,"default"));
...
sc_require("resources/main_page");Smartgraphs.authorPageDef=SC.Page.extend({authorView:SC.View.design({childViews:["instructionsWrapper","dataWrapper"],theme:"sc-ace",instructionsWrapper:SC.View.design({layout:{left:0,width:0.45},childViews:"instructionsView".w(),instructionsView:SC.View.design({classNames:"smartgraph-pane",childViews:"textWrapper".w(),textWrapper:SC.View.design({layout:{top:20,right:20,left:20},classNames:"text-wrapper".w(),childViews:"introText".w(),introText:SC.StaticContentView.design({contentBinding:"Smartgraphs.activityPageController.introText",isVisibleBinding:SC.Binding.bool("Smartgraphs.activityPageController.introText")})})})}),dataWrapper:SC.View.design({layout:{right:0,width:0.55},childViews:"dataView".w(),dataView:SC.View.design({layout:{top:4,right:4,bottom:4,left:4}})})})});

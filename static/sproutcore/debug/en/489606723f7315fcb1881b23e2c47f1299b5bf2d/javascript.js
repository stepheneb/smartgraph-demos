...
}SC.addInvokeOnceLastDebuggingInfo=function(){return;SC.ObserverSet.add=function(i,b,e,h,o,k){var c=(i)?SC.guidFor(i):"__this__";
var g=this[c];if(!g){g=this[c]=SC.CoreSet.create();g.target=i;g.isTargetSet=YES;this.targets++
}g.add(b);if(e!==undefined){if(!g.contexts){g.contexts={}}g.contexts[SC.guidFor(b)]=e
}this._membersCacheIsValid=NO;if(o!==undefined){var f=g.originatingTargets;var n=g.originatingMethods;
var a=g.originatingStacks;if(!f){f=g.originatingTargets={}}if(!n){n=g.originatingMethods={}
}if(!a){a=g.originatingStacks={}}var m=SC.guidFor(b);var d=n[m];if(d&&SC.typeOf(d)!==SC.T_ARRAY){var j=f[m];
var l=a[m];f[m]=[j,h];n[m]=[d,o];a[m]=[l,k]}else{f[m]=h;n[m]=o;a[m]=k}}};SC.ObserverSet.invokeMethods=function(){for(var p in this){if(!this.hasOwnProperty(p)){continue

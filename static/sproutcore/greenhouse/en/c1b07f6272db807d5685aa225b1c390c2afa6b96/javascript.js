...
}if(SC.BUNDLE_INFO[a]){return}SC.BUNDLE_INFO[a]={requires:["sproutcore/empty_theme"],styles:["/static/sproutcore/standard_theme/en/436311824016a77b8d960c0a2ffbf28f1ee20f39/stylesheet-packed.css","/static/sproutcore/standard_theme/en/436311824016a77b8d960c0a2ffbf28f1ee20f39/stylesheet.css"],scripts:["/static/sproutcore/standard_theme/en/436311824016a77b8d960c0a2ffbf28f1ee20f39/javascript-packed.js"]}
...
Greenhouse.fileController=SC.ObjectController.create({contentBinding:"Greenhouse.filesController.selection",contentBindingDefault:SC.Binding.single(),_content_statusDidChange:function(){var a=this.get("content");
if(a&&a.get("isPage")){this.invokeOnce(function(){Greenhouse.sendAction("fileSelectedIsAPage");
Greenhouse.sendAction("cancel")})}else{if(a&&!a.get("isPage")){this.invokeOnce(function(){Greenhouse.sendAction("fileSelectedIsNotAPage")
})}}}.observes("*content.body")});Greenhouse.filesController=SC.TreeController.create(SC.CollectionViewDelegate,{collectionViewValidateDragOperation:function(b,d,e,c,a){return SC.DRAG_ANY
...
}else{this.showDropDown()}}};Greenhouse.Design=SC.Record.extend({primaryKey:"scClass",name:SC.Record.attr(String),scClass:SC.Record.attr(String),defaults:SC.Record.attr(Object),canEdit:function(){return this._parentRecord.get("canEdit")
}.property().cacheable()});require("core");Greenhouse.Dir=SC.ChildRecord.extend({type:"Dir",childRecordNamespace:Greenhouse,name:SC.Record.attr(String),dir:SC.Record.attr(String),contents:SC.Record.toMany("Greenhouse.File",{nested:YES}),primaryKey:"id",isFile:NO,path:function(){return this.get("dir")+this.get("name")
...
}catch(exception){console.log("Couldn't eval body...")}},includesFile:function(b){if(!this.get("isFile")){var c=this.get("contents"),a;
...
Greenhouse.mixin({inspectorStates:SC.State.design({initialSubstate:"inspectorClosed",inspectorClosed:SC.State.design({parallelStatechart:"inspector",openInspector:function(a){if(a){Greenhouse.openInspectorPicker.set("anchor",a)
}this.gotoState("openInspectorPicker")},toggleDockedInspector:function(){this.gotoState("dockedInspector")
},floatInspector:function(){this.gotoState("inspectorPalette")}}),openInspectorPicker:SC.State.design({parallelStatechart:"inspector",enterState:function(){var c=Greenhouse.appPage;
...
c.setIfChanged("nowShowing",null);a.remove();this.set("anchor",null)},cancel:function(){this.gotoState("inspectorClosed")
},floatInspector:function(){this.gotoState("inspectorPalette")},toggleDockedInspector:function(){this.gotoState("dockedInspector")
}}),inspectorPalette:SC.State.design({parallelStatechart:"inspector",enterState:function(){var b=Greenhouse.appPage;
...
d.adjust("top",0);c.set("isVisible",NO)},closeInspector:function(){this.gotoState("inspectorClosed")
},toggleDockedInspector:function(){this.gotoState("dockedInspector")}}),dockedInspector:SC.State.design({parallelStatechart:"inspector",enterState:function(){var a=Greenhouse.appPage.get("inspectorDockView");
...
if(a!==Greenhouse.get("dockedLibrary")){Greenhouse.sendEvent("undock")}this.gotoState("inspectorClosed")
}})})});Greenhouse.mixin({libraryStates:SC.State.design({initialSubstate:"libraryClosed",libraryClosed:SC.State.design({parallelStatechart:"library",openLibrary:function(){this.gotoState("openLibraryPicker")
},toggleDockedLibrary:function(){this.gotoState("dockedLibrary")}}),openLibraryPicker:SC.State.design({parallelStatechart:"library",enterState:function(){var a=Greenhouse.appPage.get("libraryPicker"),b=Greenhouse.appPage.getPath("mainView.toolBar.library"),c=Greenhouse.appPage.get("libraryPickerContentView");
...
b.setIfChanged("nowShowing",null);a.remove()},cancel:function(){this.gotoState("libraryClosed")
},floatLibrary:function(){this.gotoState("libraryPalette")},toggleDockedLibrary:function(){this.gotoState("dockedLibrary")
}}),libraryPalette:SC.State.design({parallelStatechart:"library",enterState:function(){var b=Greenhouse.appPage;
...
d.adjust("top",49);c.set("isVisible",NO)},closeLibrary:function(){this.gotoState("libraryClosed")
},toggleDockedLibrary:function(){this.gotoState("dockedLibrary")}}),dockedLibrary:SC.State.design({parallelStatechart:"library",enterState:function(){var a=Greenhouse.appPage.get("libraryDockView");
...
if(a!==Greenhouse.get("dockedInspector")){Greenhouse.sendEvent("undock")}this.gotoState("libraryClosed")
}})})});Greenhouse.mixin({modalStates:SC.State.design({initialSubstate:"modalReady",modalReady:SC.State.design({parallelStatechart:"modals",newBindingPopup:function(a){Greenhouse.createBindingPopup.set("newItem",a);
this.gotoState("createBindingPopup")},newCustomView:function(){this.gotoState("addCustomView")
},editProperty:function(){this.gotoState("editProperties")},newPageElement:function(a){Greenhouse.set("newItem",a);
this.gotoState("addToPage")},openProjectPicker:function(){this.gotoState("projectPicker")
}}),projectPicker:SC.State.design({parallelStatechart:"modals",enterState:function(){var a=Greenhouse.appPage.get("projectPicker"),b=Greenhouse.appPage.getPath("mainView.toolBar.project");
a.popup(b,SC.PICKER_POINTER);a.becomeFirstResponder()},exitState:function(){var a=Greenhouse.appPage.get("projectPicker");
a.remove()},cancel:function(){this.gotoState("modalReady")},newPageFile:function(){this.gotoState("newPage")
...
this.set("newItem",null)},cancel:function(){this.gotoState("modalReady")},create:function(){var f=Greenhouse.get("newBindingFromKey"),a=Greenhouse.get("newBindingToKey"),d=this.get("newItem"),b=Greenhouse.designController.get("view"),g=Greenhouse.designController.get("content");
...
}this.gotoState("modalReady")}}),addCustomView:SC.State.design({parallelStatechart:"modals",enterState:function(){var a=Greenhouse.dialogPage.get("modal");
...
Greenhouse.viewConfigsController.refreshContent();this.gotoState("modalReady")}}),newPage:SC.State.design({parentState:"projectPicker",parallelStatechart:"modals",enterState:function(){var a=Greenhouse.dialogPage.get("modal");
...
h.commitRecord();this.gotoState("projectPicker")}}),editProperties:SC.State.design({parallelStatechart:"modals",enterState:function(){var a=Greenhouse.dialogPage.get("propertyPicker");
...
a.remove();Greenhouse.set("newItem",null);Greenhouse.set("newPageItemName","")},cancel:function(){this.gotoState("modalReady")
},add:function(){var b=Greenhouse.get("newItem"),a=Greenhouse.get("newPageItemName");
b.addItemToPage(a);this.gotoState("modalReady")}})})});Greenhouse.mixin({readyStates:SC.State.design({initialSubstate:"readyWaiting",enterState:function(){console.log("greenhouse has landed");
...
if(a){a.refresh();this.gotoState("gettingFile")}},unselectFile:function(){this.gotoState("readyWaiting")
},reloadIframe:function(){Greenhouse.filesController.set("selection",null);Greenhouse.gettingFile._firstTime=YES;
Greenhouse.iframe.location.reload();this.gotoState("iframeLoading")},resizePage:function(b){var d=b.getPath("content.size"),e={top:20,left:20,right:20,bottom:83},c=Greenhouse.get("iframe"),a;
a=c.SC.designPage.getPath("designMainPane.container");if(!d){a.set("classNames",["design"]);
a.set("layout",e)}else{a.set("classNames",[]);a.set("layout",SC.merge({centerX:0,centerY:0},d))
...
this._firstTime=NO;this.goHistoryState("pageSelected")},fileSelectedIsNotAPage:function(){this.gotoState("fileSelected")
}}),fileSelected:SC.State.design({enterState:function(){},exitState:function(){}}),pageSelected:SC.State.design({parentState:"ready",initialSubstate:"noDock",enterState:function(){},exitState:function(){},save:function(){var a,b=Greenhouse.fileController.get("content");
...
}},noDock:SC.State.design({parentState:"pageSelected",enterState:function(){var b=Greenhouse.appPage.get("dockView");
b.set("layout",{top:0,bottom:0,right:0,width:0});var a=Greenhouse.appPage.get("designAreaView");
a.set("layout",{top:0,bottom:0,right:0,left:0})},exitState:function(){},toggleDockedLibrary:function(){this.gotoState("docked")
},toggleDockedInspector:function(){this.gotoState("docked")}}),docked:SC.State.design({parentState:"pageSelected",enterState:function(){var b=Greenhouse.appPage.get("dockView");
b.set("layout",{top:0,bottom:0,right:0,width:230});var a=Greenhouse.appPage.get("designAreaView");
a.set("layout",{top:0,left:0,right:230,bottom:0})},exitState:function(){},undock:function(){this.gotoState("noDock")
...
},fetchTargetsDidComplete:function(){this.gotoState("chooseApp")}}),chooseApp:SC.State.design({enterState:function(){var a=Greenhouse.getPath("mainPage.mainPane.container");
...
Greenhouse.viewConfigsController.reload();this.gotoState("loading")}}),ready:SC.State.plugin("Greenhouse.readyStates"),iframeLoading:SC.State.design({enterState:function(){var a=Greenhouse.getPath("mainPage.mainPane.container");
a.set("nowShowing",Greenhouse.getPath("appPage.mainView"))},exitState:function(){},iframeLoaded:function(){this.gotoState("syncRunLoops")
}}),syncRunLoops:SC.State.design({enterState:function(){this._setupRunLoops();this._grabDropTargets();
this._setupGreenhouse();this._setupEventBlocker();this.gotoState("readyWaiting")},exitState:function(){},_setupRunLoops:function(){var c=Greenhouse.get("iframe"),a,f,d,e,b=SC;
f=function(){var g=b.RunLoop.currentRunLoop;if(!g){g=b.RunLoop.currentRunLoop=b.RunLoop.runLoopClass.create()
}g.beginRunLoop();return b.RunLoop};e=function(){var g=b.RunLoop.currentRunLoop;if(!g){throw"SC.RunLoop.end() called outside of a runloop!"
}g.endRunLoop();return b.RunLoop};a=function(){var g=c.SC.RunLoop.currentRunLoop;
if(!g){g=c.SC.RunLoop.currentRunLoop=c.SC.RunLoop.runLoopClass.create()}g.beginRunLoop();
return c.SC.RunLoop};d=function(){var g=c.SC.RunLoop.currentRunLoop;if(!g){throw"SC.RunLoop.end() called outside of a runloop!"
}g.endRunLoop();return c.SC.RunLoop};b.RunLoop.begin=function(){var g=f();a();return g
};c.SC.RunLoop.begin=function(){var g=a();f();return g};c.SC.RunLoop.end=function(){e();
return d()};b.RunLoop.end=function(){d();return e()}},_grabDropTargets:function(){var c=Greenhouse.get("iframe"),b,a,f=Greenhouse.appPage.get("webView");

...
}}else{this.gotoState(d,c,YES)}},sendEvent:function(g,f,e){var h=NO,c=this.get("currentStates").slice(),a=0,d=0,b=null;
if(this._sendEventLocked||this._goStateLocked){this._pendingSentEvents.push({event:g,sender:f,context:e});
return}this._sendEventLocked=YES;a=c.get("length");for(;d<a;d+=1){h=NO;b=c[d];if(!b.get("isCurrentState")){continue
}while(!h&&b){if(b.tryToPerform){h=b.tryToHandleEvent(g,f,e)}if(!h){b=b.get("parentState")
}}}this._sendEventLocked=NO;this._flushPendingSentEvents();return b},_createStateChain:function(b){var a=[];

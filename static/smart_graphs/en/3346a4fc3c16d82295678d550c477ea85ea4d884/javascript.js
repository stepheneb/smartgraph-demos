...
SmartGraphs.dataPointController=SC.ObjectController.create({});SmartGraphs.dataSeriesController=SC.ArrayController.create({xs:function(){return this.get("content").map(function(a){return a.x
})}.property("content").cacheable(),ys:function(){return this.get("content").map(function(a){return a.y
})}.property("content").cacheable(),_contentDidChange:function(){console.log("_contentDidChange");
this.notifyPropertyChange("content")}.observes(".content.[]")});SmartGraphs.Axes=SC.Record.extend({xMax:SC.Record.attr(Number),xMin:SC.Record.attr(Number),yMax:SC.Record.attr(Number),yMin:SC.Record.attr(Number),yScale:SC.Record.attr(Number),xScale:SC.Record.attr(Number)});
sc_require("models/axes");SmartGraphs.Axes.FIXTURES=[{guid:1,xMin:0,xMax:5,yMin:0,yMax:25,xScale:1,yScale:5}];
...
sc_require("models/data_point");SmartGraphs.DataPoint.FIXTURES=(function(){var b=[],c;
for(var a=0;a<24;a++){c={};c.guid=(a+1)+"";c.x=a/5;c.y=a+Math.random();c.series="series-1";
b.push(c)}return b}());SmartGraphs.DataSeries=SC.Record.extend({series:SC.Record.attr(Array)});
sc_require("models/data_series");SmartGraphs.DataSeries.FIXTURES=[{guid:1,series:(function(){var a=[];
for(var b=0;b<25;b++){a.push({x:b/5,y:b+Math.random(),set:function(c,e){this[c]=parseFloat(e)
}})}return a}())}];
/*
...
},render:function(c,a){var l=this.get("parentView");var o=l.get("raphaelObject");
var k=this.get("padding");var h=l.get("layout");var e=this.get("xMin"),g=this.get("xMax"),m=this.get("yMin"),f=this.get("yMax");
if(o&&k&&h&&(e!==undefined)&&(g!==undefined)&&(m!==undefined)&&(f!==undefined)){var n=h.height,b=h.width;
var j=b-k.left-k.right;var i=n-k.top-k.bottom;if(this._x){this._x.remove()}this._x=o.g.axis(k.left,k.top+i,j,0,g,g,0);
if(this._y){this._y.remove()}this._y=o.g.axis(k.left,k.top+i,i,0,f,f,1);this.renderChildViews(c,a)
...
if(c){b=b.begin("div").addClass("inner").end()}this.$(".inner").html(a)}});SmartGraphs.RaphaelView=SC.View.extend({raphaelObject:null,_childViewForId:{},_lastViewForMouseMove:null,didCreateLayer:function(){var c=Raphael;
...
if(a){a.attr(this.NO_HIGHLIGHT_ATTR)}},render:function(e,h){var l=this.get("parentView");
var f=l.get("raphaelObject");var p=this.get("padding");var z=l.get("layout");var A=this.get("xMin"),B=this.get("xMax"),a=this.get("yMin"),b=this.get("yMax");
...
v<o;v++){var s=n.objectAt(v);var m=p.left+(s.x*E);var j=p.top+w-(s.y*c);var u=f.circle(m,j).attr(this.NO_HIGHLIGHT_ATTR);
if(D.contains(s)){u.attr(this.SELECTED_ATTR)}else{u.attr(this.NOT_SELECTED_ATTR)}u.node.id=this.id+"-data-point-%@".fmt(v);
this._raphaelObjForId[u.node.id]=u;this._dataPointForId[u.node.id]=s}}}});SmartGraphs.mainPage=SC.Page.design({mainPane:SC.MainPane.design({layout:{width:960,height:720},childViews:"textView bottomLeftView graphView tableView".w(),textView:SmartGraphs.HTMLView.design({layout:{left:20,top:20,width:455,height:335},classNames:["smartgraph-pane"],html:"<h2>Hi There!</h2> Try <a href='http://sproutcore.com'>Sproutcore</a> for all your: <ul> <li> Software Needs <li> Graphing needs <li> Macrobiotic diets </ul>"}),bottomLeftView:SC.LabelView.design({layout:{left:20,top:365,width:455,height:335},classNames:["smartgraph-pane"]}),graphView:SmartGraphs.RaphaelView.design({layout:{left:485,top:20,width:455,height:335},childViews:"axesView series1View".w(),classNames:["smartgraph-pane"],axesView:SmartGraphs.AxesView.design({xMinBinding:"SmartGraphs.axesController.xMin",xMaxBinding:"SmartGraphs.axesController.xMax",yMinBinding:"SmartGraphs.axesController.yMin",yMaxBinding:"SmartGraphs.axesController.yMax",xScaleBinding:"SmartGraphs.axesController.xScale",yScaleBinding:"SmartGraphs.axesController.yScale",paddingBinding:"SmartGraphs.axesController.padding"}),series1View:SmartGraphs.SeriesView.design({xMinBinding:"SmartGraphs.axesController.xMin",xMaxBinding:"SmartGraphs.axesController.xMax",yMinBinding:"SmartGraphs.axesController.yMin",yMaxBinding:"SmartGraphs.axesController.yMax",xScaleBinding:"SmartGraphs.axesController.xScale",yScaleBinding:"SmartGraphs.axesController.yScale",paddingBinding:"SmartGraphs.axesController.padding",controllerBinding:"SmartGraphs.dataSeriesController",contentBinding:"SmartGraphs.dataSeriesController.arrangedObjects",selectionBinding:"SmartGraphs.dataSeriesController.selection"})}),tableView:SC.CollectionView.design({layout:{left:485,top:365,width:455,height:335},classNames:["smartgraph-pane"],childViews:["labelsView","scrollerView"],labelsView:SC.CollectionView.design({layout:{left:0,top:0,width:455,height:30},childViews:["xsLabel","ysLabel"],xsLabel:SC.LabelView.design({layout:{left:30,width:20,top:7,height:20},displayValue:"x"}),ysLabel:SC.LabelView.design({layout:{left:90,width:20,top:7,height:20},displayValue:"y"})}),scrollerView:SC.ScrollView.design({layout:{left:0,top:30,width:455,height:305},borderStyle:SC.BORDER_NONE,contentView:SC.CollectionView.design({childViews:["xsView","ysView"],xHeightBinding:SC.Binding.from(".xsView.height").oneWay(),yHeightBinding:SC.Binding.from(".ysView.height").oneWay(),height:function(){return Math.max(this.get("xHeight"),this.get("yHeight"))
...
var a=SmartGraphs.store.find(SmartGraphs.DataSeries,"1");var b=SmartGraphs.store.find(SmartGraphs.Axes,"1");
SmartGraphs.dataSeriesController.set("content",a.get("series"));SmartGraphs.axesController.set("content",b)

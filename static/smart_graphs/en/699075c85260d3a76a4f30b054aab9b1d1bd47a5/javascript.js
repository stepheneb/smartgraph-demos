...
SmartGraphs.dataSeriesController=SC.ArrayController.create({xs:function(){return this.get("content").map(function(a){return a.x
})}.property("content").cacheable(),ys:function(){return this.get("content").map(function(a){return a.y
})}.property("content").cacheable(),_contentDidChange:function(){console.log('dataSeriesController invalidating property "content"');
this.notifyPropertyChange("content")}.observes(".content.[]")});SmartGraphs.DataSeries=SC.Record.extend({series:SC.Record.attr(Array)});
sc_require("models/data_series");SmartGraphs.DataSeries.FIXTURES=[{guid:1,series:(function(){var a=[];
for(var b=0;b<25;b++){a.push({x:b/5,y:b})}return a}())}];
/*
...
return V};SmartGraphs.GraphView=SC.View.extend({highlighted:null,classNames:["graph-view"],displayProperties:["selection","series"],_isGraphed:false,_raphaelObjForId:{},_highlightedNodeId:undefined,NO_HIGHLIGHT_ATTR:{opacity:0.5,r:4},HIGHLIGHT_ATTR:{opacity:1,r:5},DATA_POINT_ID_MATCHER:/data-point-(\d+)/,render:function(b,f){var a=this.get("series");
...
if(c){b=b.begin("div").addClass("inner").end()}this.$(".inner").html(a)}});SmartGraphs.mainPage=SC.Page.design({mainPane:SC.MainPane.design({layout:{width:960,height:720},childViews:"textView bottomLeftView graphView tableView".w(),textView:SmartGraphs.HTMLView.design({layout:{left:20,top:20,width:455,height:335},classNames:["smartgraph-pane"],html:"<h2>Hi There!</h2> Try <a href='http://sproutcore.com'>Sproutcore</a> for all your: <ul> <li> Software Needs <li> Graphing needs <li> Macrobiotic diets </ul>"}),bottomLeftView:SC.LabelView.design({layout:{left:20,top:365,width:455,height:335},classNames:["smartgraph-pane"]}),graphView:SmartGraphs.GraphView.design({seriesBinding:"SmartGraphs.dataSeriesController.arrangedObjects",selectionBinding:"SmartGraphs.dataSeriesController.selection",layout:{left:485,top:20,width:455,height:335},classNames:["smartgraph-pane"]}),tableView:SC.CollectionView.design({layout:{left:485,top:365,width:455,height:335},classNames:["smartgraph-pane"],childViews:["labelsView","scrollerView"],labelsView:SC.CollectionView.design({layout:{left:0,top:0,width:455,height:30},childViews:["xsLabel","ysLabel"],xsLabel:SC.LabelView.design({layout:{left:25,width:20,top:7,height:20},displayValue:"x"}),ysLabel:SC.LabelView.design({layout:{left:75,width:20,top:7,height:20},displayValue:"y"})}),scrollerView:SC.ScrollView.design({layout:{left:0,top:30,width:455,height:305},borderStyle:SC.BORDER_NONE,contentView:SC.CollectionView.design({childViews:["xsView","ysView"],xsView:SC.ListView.design({layout:{left:10,top:0,width:40},isEditable:YES,canEditContent:NO,contentValueKey:"x",contentBinding:"SmartGraphs.dataSeriesController.arrangedObjects",selectionBinding:"SmartGraphs.dataSeriesController.selection",rowHeight:18}),ysView:SC.ListView.design({layout:{left:60,top:0,width:40},isEditable:YES,canEditContent:NO,contentValueKey:"y",contentBinding:"SmartGraphs.dataSeriesController.arrangedObjects",selectionBinding:"SmartGraphs.dataSeriesController.selection",rowHeight:18})}),_subContentHeightDidChange:function(){var c=this.get("contentView");
var g=c.get("xsView");var f=c.get("ysView");var a=g.get("calculatedHeight")+g.get("layout").top;
var b=f.get("calculatedHeight")+f.get("layout").top;var e=SC.copy(c.get("layout"));
e.height=Math.max(a,b);c.set("layout",e);console.log("_subContentHeightDidChange")
...
var a=SmartGraphs.store.find(SmartGraphs.DataSeries,"1");SmartGraphs.dataSeriesController.set("content",a.get("series"))

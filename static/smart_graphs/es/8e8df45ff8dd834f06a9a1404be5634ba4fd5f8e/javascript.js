...
SmartGraphs.DataSeries=SC.Record.extend({series:SC.Record.attr(Array)});
/*
...
return V};SmartGraphs.GraphView=SC.View.extend({seriesBinding:"SmartGraphs.mainPage.mainPane.listView.content",selectionBinding:"SmartGraphs.mainPage.mainPane.listView.selection",highlighted:null,classNames:["graph-view"],displayProperties:["selection","series"],_isGraphed:false,_raphaelObjForId:{},_highlightedNodeId:undefined,NO_HIGHLIGHT_ATTR:{opacity:0.5,r:4},HIGHLIGHT_ATTR:{opacity:1,r:5},DATA_POINT_ID_MATCHER:/data-point-(\d+)/,render:function(b,f){var a=this.get("series");
...
}}});SmartGraphs.mainPage=SC.Page.design({mainPane:SC.MainPane.design({childViews:"listView graphView".w(),listView:SC.ListView.design({layout:{left:10,top:10,width:600,height:200},rowHeight:18}),graphView:SmartGraphs.GraphView.design({layout:{left:10,top:220,width:600,height:400}})})});
SmartGraphs.main=function main(){SmartGraphs.getPath("mainPage.mainPane").append();
var a=SmartGraphs.store.find(SmartGraphs.DataSeries,"1");SmartGraphs.setPath("mainPage.mainPane.listView.content",a.get("series"))

...
}if(SC.BUNDLE_INFO[a]){return}SC.BUNDLE_INFO[a]={requires:["sproutcore/empty_theme"],styles:["/smartgraph-demos/static/sproutcore/standard_theme/es/ae78e682efa8d92ab1f23de86e1e7855014416c6/stylesheet-packed.css","/smartgraph-demos/static/sproutcore/standard_theme/es/ae78e682efa8d92ab1f23de86e1e7855014416c6/stylesheet.css"],scripts:["/smartgraph-demos/static/sproutcore/standard_theme/es/ae78e682efa8d92ab1f23de86e1e7855014416c6/javascript-packed.js"]}
...
a.addObserver("y",this,this._valuesDidChange)},this)}.observes(".[]")});SmartGraphs.Axes=SC.Record.extend({xMax:SC.Record.attr(Number),xMin:SC.Record.attr(Number),yMax:SC.Record.attr(Number),yMin:SC.Record.attr(Number),ySteps:SC.Record.attr(Number),xSteps:SC.Record.attr(Number)});
...
return V};SmartGraphs.AxesView=SC.View.extend({displayProperties:"xMin xMax yMin yMax padding".w(),didCreateLayer:function(){this.$().css("zIndex","-1")
},render:function(c,a){var n=this.get("parentView");var q=n.get("raphaelObject");
var m=this.get("padding");var i=n.get("layout");var e=this.get("xMin"),h=this.get("xMax"),o=this.get("yMin"),f=this.get("yMax"),g=this.get("xSteps"),j=this.get("ySteps");
if(q&&m&&i&&(e!==undefined)&&(h!==undefined)&&(o!==undefined)&&(f!==undefined)){var p=i.height,b=i.width;
var l=b-m.left-m.right;var k=p-m.top-m.bottom;if(this._x){this._x.remove()}this._x=q.g.axis(m.left,m.top+k,l,0,h,g,0);
if(this._y){this._y.remove()}this._y=q.g.axis(m.left,m.top+k,k,0,f,j,1);this.renderChildViews(c,a)
}}});SmartGraphs.EditableListItemView=SC.ListItemView.extend({inlineEditorShouldBeginEditing:function(){return YES
}});SmartGraphs.GraphView=SC.View.extend({highlighted:null,classNames:["graph-view"],displayProperties:["selection","series"],_isGraphed:false,_raphaelObjForId:{},_highlightedNodeId:undefined,NO_HIGHLIGHT_ATTR:{opacity:0.5,r:4},HIGHLIGHT_ATTR:{opacity:1,r:5},DATA_POINT_ID_MATCHER:/data-point-(\d+)/,render:function(b,f){var a=this.get("series");
var c=this.get("selection");var e;if(f){b=b.begin("div").addClass("graph").end()}if(a&&!this._isGraphed){this._renderGraph(this.$(".graph")[0]);
console.log("rendered");this._isGraphed=true}},_renderGraph:function(p){var m={top:20,right:20,bottom:20,left:20};
var v=5,a=25;var u=this.$().height();var j=this.$().width();var l=Raphael;var n=l(p,j,u);
var c=j-m.left-m.right;var s=u-m.top-m.bottom;var z=c/v;var b=s/a;n.g.axis(m.left,m.top+s,c,0,v,v,0);
n.g.axis(m.left,m.top+s,s,0,a,a,1);var g=this.get("series");for(var t=0,k=g.get("length");
t<k;t++){var o=g.objectAt(t);var f=m.left+(o.x*z);var e=m.top+s-(o.y*b);var q=n.circle(f,e).attr({stroke:"#aa0000",fill:"#aa0000"}).attr(this.NO_HIGHLIGHT_ATTR);
q.node.id="data-point-%@".fmt(t);this._raphaelObjForId[q.node.id]=q}this._r=n},mouseDown:function(b){var a=this.DATA_POINT_ID_MATCHER.exec(b.target.id);
if(a){console.log("hello from data point"+a[1])}},mouseMoved:function(g){var b=g.target;
var f;var c;var a=this.DATA_POINT_ID_MATCHER.exec(b.id);if(a){f=b.id;c=parseInt(a[1],10)
}if(f!==this._highlightedNodeId){if(this._highlightedNodeId){this._raphaelObjForId[this._highlightedNodeId].attr(this.NO_HIGHLIGHT_ATTR);
this._highlightedNodeId=undefined}if(f){this._raphaelObjForId[f].attr(this.HIGHLIGHT_ATTR);
this._highlightedNodeId=f}this.set("highlighted",(c===undefined)?null:this.get("series").objectAt(c))
}}});SmartGraphs.HTMLView=SC.View.extend({html:"",displayProperties:["html"],classNames:["html-view"],render:function(b,c){var a=this.get("html");
console.log("rendering HTMLView: this.$('.inner') = "+this.$(".inner"));b=b.begin("div").addClass("inner").html(a).end()
},didCreateLayer:function(){console.log("HTMLView didCreateLayer()")}});SC.QuestionView=SC.StackedView.extend(SC.StaticLayout,{layout:{top:0,left:0,right:0},classNames:["question","open-response-question"],contentDisplayProperties:"prompt".w(),prompt:"[prompt]",useStaticLayout:NO,childViews:"promptView inputView checkButton".w(),promptView:SC.StaticContentView.design({contentBinding:"*parentView.prompt"}),inputView:SC.View.design(SC.StaticLayout,{layout:{left:20,top:5,right:20,height:95},useStaticLayout:YES,childViews:"textFieldView".w(),textFieldView:SC.TextFieldView.design({classNames:"question-input",isTextArea:YES})}),checkButton:SC.ButtonView.design(SC.StaticLayout,{useStaticLayout:YES,layout:{top:10,left:20,width:120},title:"Check Answer",answerBinding:"*parentView.inputView.textFieldView.value",action:function(){SC.AlertPane.info('You said "'+this.get("answer")+"\"; I won't try to check this answer for you until we've thought more carefully about UI and data modeling issues")
}})});SmartGraphs.RaphaelView=SC.View.extend({raphaelObject:null,_childViewForId:{},_lastViewForMouseMove:null,didCreateLayer:function(){var c=Raphael;
var b=this.get("layout");this.$().append("<div class='graph'></div>");var a=c(this.$(".graph")[0],b.width,b.height);
...
if(a){return a.mouseDown(b)}}});SmartGraphs.SeriesView=SC.View.extend({_raphaelObjForId:{},_dataPointForId:{},_highlightedPoint:null,NO_HIGHLIGHT_ATTR:{r:4},HIGHLIGHT_ATTR:{r:5},SELECTED_ATTR:{stroke:"#aa0000",fill:"#aa0000",opacity:0.8},NOT_SELECTED_ATTR:{stroke:"#BFADA7",fill:"#BFADA7",opacity:0.5},DATA_POINT_ID_MATCHER:"",id:"",init:function(){arguments.callee.base.apply(this,arguments);
...
},mouseDown:function(c){var f=this._dataPointForId[c.target.id];if(f){var a=this.get("controller");
var b=this.get("selection");if(b.contains(f)){a.deselectObject(f)}else{a.selectObject(f,YES)
...
if(a){a.attr(this.NO_HIGHLIGHT_ATTR)}},render:function(e,h){console.log("rendering");
var l=this.get("parentView");var f=l.get("raphaelObject");var p=this.get("padding");
var z=l.get("layout");var A=this.get("xMin"),B=this.get("xMax"),a=this.get("yMin"),b=this.get("yMax");
if(f&&p&&z&&(A!==undefined)&&(B!==undefined)&&(a!==undefined)&&(b!==undefined)){var q=z.height,t=z.width;
var g=t-p.left-p.right;var w=q-p.top-p.bottom;var E=g/B;var c=w/b;var C=this._raphaelObjForId;
for(var k in C){if(C.hasOwnProperty(k)){C[k].remove()}}this._raphaelObjForId={};this._dataPointForId={};
var n=this.get("content");var D=this.get("selection");for(var v=0,o=n.get("length");
v<o;v++){var s=n.objectAt(v);var m=p.left+(s.get("x")*E);var j=p.top+w-(s.get("y")*c);
var u=f.circle(m,j).attr(this.NO_HIGHLIGHT_ATTR);if(D.contains(s)){u.attr(this.SELECTED_ATTR)
}else{u.attr(this.NOT_SELECTED_ATTR)}u.node.id=this.id+"-data-point-%@".fmt(v);this._raphaelObjForId[u.node.id]=u;
this._dataPointForId[u.node.id]=s}}}});SmartGraphs.mainPage=SC.Page.design({introView:SC.StaticContentView.design({classNames:["sg-question"],content:"Maria ran practice laps around the track. Her coach wrote the distance she ran after each minute. These data are shown in the scatterplot and the table at right.<br><br>Click on a point in the scatterplot where Maria stopped to talk with her coach."}),followupView:SC.View.design({childViews:"question1".w(),classNames:"sg-question",question1:SC.QuestionView.design({useStaticLayout:YES,prompt:"For how long did Maria talk with her coach?"})}),topperView:SC.StaticContentView.design({classNames:["sg-question"],content:"Did you notice that Maria's coach must have written down her position three times while they were talking?<br><br>About how long did it take Maria to start running again after the third time?"}),mainPane:SC.MainPane.design({layout:{width:960,height:720},childViews:"promptView graphView tableView".w(),promptView:SC.View.design({layout:{left:20,top:20,width:455,height:680},classNames:["smartgraph-pane"],childViews:"tabView nextButton backButton".w(),tabView:SC.TabView.design({layout:{top:30,bottom:5,left:5,right:5},items:[{title:"Maria's Run",value:"SmartGraphs.mainPage.introView"},{title:"A Pep Talk",value:"SmartGraphs.mainPage.followupView"},{title:"Back to Running",value:"SmartGraphs.mainPage.topperView"}],itemTitleKey:"title",itemValueKey:"value",nowShowing:"SmartGraphs.mainPage.introView"}),nextButton:SC.ButtonView.design({layout:{top:620,left:325,width:80},title:"Next",target:"SmartGraphs.mainPage.mainPane.promptView",action:"nextTab",nowShowingBinding:"SmartGraphs.mainPage.mainPane.promptView.tabView.nowShowing",isEnabled:function(){var a=this.get("nowShowing");
return(a!=="SmartGraphs.mainPage.topperView")}.property("nowShowing")}),backButton:SC.ButtonView.design({layout:{top:620,left:50,width:80},title:"Back",target:"SmartGraphs.mainPage.mainPane.promptView",action:"backTab",nowShowingBinding:"SmartGraphs.mainPage.mainPane.promptView.tabView.nowShowing",isEnabled:function(){var a=this.get("nowShowing");
return(a!=="SmartGraphs.mainPage.introView")}.property("nowShowing")}),nextTab:function(){var a=this.get("tabView");
var b=a.get("nowShowing");switch(b){case"SmartGraphs.mainPage.introView":a.set("nowShowing","SmartGraphs.mainPage.followupView");
break;case"SmartGraphs.mainPage.followupView":a.set("nowShowing","SmartGraphs.mainPage.topperView");
break}},backTab:function(){var a=this.get("tabView");var b=a.get("nowShowing");switch(b){case"SmartGraphs.mainPage.topperView":a.set("nowShowing","SmartGraphs.mainPage.followupView");
break;case"SmartGraphs.mainPage.followupView":a.set("nowShowing","SmartGraphs.mainPage.introView");
break}}}),graphView:SmartGraphs.RaphaelView.design({layout:{left:485,top:20,width:455,height:335},childViews:"axesView series1View".w(),classNames:["smartgraph-pane"],axesView:SmartGraphs.AxesView.design({xMinBinding:"SmartGraphs.axesController.xMin",xMaxBinding:"SmartGraphs.axesController.xMax",yMinBinding:"SmartGraphs.axesController.yMin",yMaxBinding:"SmartGraphs.axesController.yMax",xStepsBinding:"SmartGraphs.axesController.xSteps",yStepsBinding:"SmartGraphs.axesController.ySteps",paddingBinding:"SmartGraphs.axesController.padding"}),series1View:SmartGraphs.SeriesView.design({xMinBinding:"SmartGraphs.axesController.xMin",xMaxBinding:"SmartGraphs.axesController.xMax",yMinBinding:"SmartGraphs.axesController.yMin",yMaxBinding:"SmartGraphs.axesController.yMax",xScaleBinding:"SmartGraphs.axesController.xScale",yScaleBinding:"SmartGraphs.axesController.yScale",paddingBinding:"SmartGraphs.axesController.padding",controllerBinding:"SmartGraphs.dataSeriesController",contentBinding:"SmartGraphs.dataSeriesController.arrangedObjects",selectionBinding:"SmartGraphs.dataSeriesController.selection"})}),tableView:SC.View.design({layout:{left:485,top:365,width:455,height:335},classNames:["smartgraph-pane"],childViews:["labelsView","scrollerView"],labelsView:SC.View.design({layout:{left:0,top:0,width:455,height:30},childViews:["xsLabel","ysLabel"],xsLabel:SC.LabelView.design({layout:{left:10,width:40,top:7,height:20},displayValue:"time"}),ysLabel:SC.LabelView.design({layout:{left:70,width:50,top:7,height:20},displayValue:"distance"})}),scrollerView:SC.ScrollView.design({layout:{left:0,top:30,width:455,height:305},borderStyle:SC.BORDER_NONE,contentView:SC.View.design({childViews:["xsView","ysView"],xHeightBinding:SC.Binding.from(".xsView.height").oneWay(),yHeightBinding:SC.Binding.from(".ysView.height").oneWay(),height:function(){return Math.max(this.get("xHeight"),this.get("yHeight"))
...
SmartGraphs.main=function main(){SmartGraphs.getPath("mainPage.mainPane").append();
var a=SmartGraphs.store.find(SmartGraphs.DataSeries,"series-1");var b=SmartGraphs.store.find(SmartGraphs.Axes,"1");
SmartGraphs.dataSeriesController.set("content",a.get("points"));SmartGraphs.axesController.set("content",b)

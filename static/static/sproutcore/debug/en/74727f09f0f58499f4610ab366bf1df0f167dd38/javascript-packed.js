...
}if(r==="document"){continue}if(h.type&&h.type==="file"){if(r==="selectionStart"||r==="selectionEnd"){continue
}}s=h[r];if(r==="SproutCore"){r="SC"}j=SC.typeOf(s);if(j===SC.T_FUNCTION){if(!s.displayName){m[e]=r;
...
SC.mapDisplayNames(s,a+1,m,f,q)}}}}m.pop()};var SC=SC||{};if(!SC.LOG_RUNLOOP_INVOCATIONS){SC.LOG_RUNLOOP_INVOCATIONS=false
